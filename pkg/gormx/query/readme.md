...
表格列表接口基础路径：`GET /workspace/api/v1/table/search/{full-code-path}`，下面所有示例的 query 都拼在该路径后。分页排序统一使用 `page`、`page_size`、`sorts`；数组形态也支持 `sort[]`。业务筛选参数使用 Request 字段自己的 `form` 名，例如 `status=启用&name=张三`。
...
```
?page=1&page_size=20&sorts=status,-created_at
```

### 组合示例（分页 + 排序 + 业务筛选）

```
?page=2&page_size=10&sorts=-created_at,id&status=已发布&title=会议&start_time=2026-04-21 00:00:00&end_time=2026-04-21 23:59:59
```
...
```
GET /workspace/api/v1/table/search/luobei/myapp/tables/hr_resume_list?page=1&page_size=20&sorts=-updated_at&job_department=技术&job_title=工程师&status=待筛选
```
...
| `Page` | int | 页码，从 1 开始 |
| `PageSize` | int | 每页条数 |
| `Sorts` | string | 排序，格式：`-field1,field2`；减号表示降序，裸字段表示升序 |
| `Sort` | []string | 重复 query 形式：`sort=-field1&sort=field2`，和 `Sorts` 等价 |
| `SortArray` | []string | 数组 query 形式：`sort[]=-field1&sort[]=field2`，和 `Sorts` 等价 |
...
func MyList(ctx *app.Context, resp response.Response) error {
    ...
    var list []MyModel
    return resp.Table(&list, queryDB, &MyModel{}, &req.PageSortReq).Build()
}
```

- `Table` 只做 Count、排序、Offset、Limit、Find 和分页信息写回。
- `Table` 不处理业务筛选；所有 `Where` / `Joins` / `Preload` 都写在 `Build()` 之前。
...
- 前端常用：`sorts=-updated_at,name`；数组形态可用 `sort[]=-updated_at&sort[]=name`（减号表示 desc，裸字段表示 asc）。
- 后端 `PageSortReq.Sorts` 解析后得到 ORDER BY 子句（列名会做安全校验并加反引号）。
...
Table 请求统一使用 `page`、`page_size`、`sorts` 做分页排序；数组形态可用 `sort[]`。筛选字段按 Request 字段名直接出现在 Query 里，并由后端 Handler 手写 `Where`。
...
| `sorts` | string | 排序：`-field1,field2`；减号表示 desc，裸字段表示 asc |
| `sort[]` | string[] | 数组排序：`sort[]=-field1&sort[]=field2`，和 `sorts` 等价 |
...
- 多列排序：前端把 `sorts` 拼成 `-field1,field2`，和 backend `ParseSortFields` / `GetSorts()` 对齐。
...
2. **后端**：Req 嵌入 `query.PageSortReq`，筛选字段显式声明并经 `ShouldBind` 绑定；Handler 里手写 `Where` / `Joins` / `Preload`，再把 queryDB 和 `&req.PageSortReq` 传给 `resp.Table(..., ...).Build()`。
