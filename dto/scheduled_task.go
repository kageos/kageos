...
type CreateScheduledTaskReq struct {
	...
	Action            string          `json:"action"`                            // 执行动作：空/execute、table_create、table_update、table_delete
	...
	NotifyUsers       []string        `json:"notify_users"`                      // 执行完成后通知的用户名列表
	NotifyDepartments []string        `json:"notify_departments"`                // 执行完成后通知的部门 full_code_path 列表
	NotifyOn          string          `json:"notify_on"`                         // none/all/success/failed，空值在有接收人时按 all
}
