...
type GetFunctionGroupInfoResp struct {
	...
	// 快照信息（方便排查问题）
	FullGroupCode string        `json:"full_group_code" example:"/luobei/testgroup/tools/tools_cashier"` // 完整函数组代码
	GroupCode     string        `json:"group_code" example:"tools_cashier"`                               // 函数组代码
	GroupName     string        `json:"group_name" example:"收银工具"`                                        // 函数组名称
	FullPath      string        `json:"full_path" example:"/luobei/testgroup/tools"`                      // 完整路径
