...
type RecordTableOperateLogReq struct {
	...
	Body        json.RawMessage `json:"body"`         // 请求体（OnTableAddRow 需要）
	Updates     json.RawMessage `json:"updates"`      // 更新的字段和值（OnTableUpdateRow 需要）
	OldValues   json.RawMessage `json:"old_values"`   // 更新前的值（OnTableUpdateRow 需要）
