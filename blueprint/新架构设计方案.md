...
6. **实用主义（Pragmatism）**：**允许 Presentation Layer 直接读取 State**（通过 Getter/Computed），不必强制走 Application -> Domain 流程，保持灵活性。只有**修改状态**或**触发业务逻辑**时，必须通过 Application Service。
...
用户信息管理分为两部分：
1. **认证用户（Current User）**：当前登录用户的身份验证、Token 管理、个人信息。
2. **用户目录（User Directory）**：系统中其他用户的信息查询（用于 UI 展示，如创建人、负责人等），需要缓存和防抖。
...
```typescript
// domain/interfaces/IUserDirectoryService.ts
import type { UserInfo } from '@/types'

export interface IUserDirectoryService {
  getUserByUsername(username: string): Promise<UserInfo | null>
  getUsersByUsernames(usernames: string[]): Promise<Map<string, UserInfo>>
  // 获取缓存统计
  getCacheStats(): any
}
```
...
#### 6.3.1 Infrastructure Layer 实现

复用现有的 Pinia Store 逻辑，将其封装为符合接口的服务。

```typescript
// infrastructure/auth/AuthServiceImpl.ts
...
export class AuthServiceImpl implements IAuthService {
  ...
  async initAuth() {
    await this.store.initAuth()
  }
}

// infrastructure/userDirectory/UserDirectoryServiceImpl.ts
import { useUserInfoStore } from '@/stores/userInfo'
import type { IUserDirectoryService } from '@/domain/interfaces/IUserDirectoryService'

export class UserDirectoryServiceImpl implements IUserDirectoryService {
  private store = useUserInfoStore()

  async getUserByUsername(username: string) {
    return await this.store.getUserInfo(username)
  }

  async getUsersByUsernames(usernames: string[]) {
    return await this.store.batchGetUserInfo(usernames)
  }
...
#### 6.3.2 Application Layer 集成

在 `WorkspaceApplicationService` 中集成认证检查。
...
export class WorkspaceApplicationService {
  constructor(
    private domainService: WorkspaceDomainService,
    private authService: IAuthService, // 注入认证服务
    private eventBus: IEventBus
  ) {}
