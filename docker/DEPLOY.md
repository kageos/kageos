...
项目使用 **Podman-in-Docker** 架构：容器引擎（Podman）内嵌在 backend 容器中，不依赖宿主机的 Docker daemon 来管理用户应用容器。拉代码 → 配 IP → `docker compose up` → 搞定。
...
docker compose logs -f mysql        # 仅 MySQL
...
docker compose down -v              # 停止并删除容器和数据卷（⚠️ 危险）

# 重新构建（代码更新后）
docker compose build backend        # 重建后端
docker compose build web            # 重建前端
...
# 查看内嵌 Podman 状态
docker compose exec backend podman info
docker compose exec backend podman ps -a
docker compose exec backend podman images
```
...
docker compose exec backend podman images | grep ai-agent-os

# 如果镜像不存在，手动触发构建
docker compose exec backend podman build -t ai-agent-os:latest -f /app/app-base/Dockerfile /app/app-base/

# 查看用户应用容器日志
docker compose exec backend podman logs <container-name>
```
