...
本文用于让新同事或大模型快速理解 AI-Agent-OS 当前的生成与运行闭环：工作台负责对话和操作入口，tools 是大模型的受控动作集合，文档和 prompt 是行为规程，SDK 是应用协议与编译校验层，runtime/app-server/front-end 根据 SDK 产出的 schema 完成部署、注册、渲染和调用。

一句话概括：

> 大模型不是直接“随便写一个网站”，而是在工作台中读取文档和当前目录上下文，通过 tools 修改 `namespace` 中的 Go 代码；SDK 把 Go 结构体、路由、模板和 widget tag 编译成函数 schema；runtime 等待新版本启动并校验成功后，app-server 保存函数资产；前端再按 schema 渲染 Form/Table/Chart，Agent 也可继续通过 run_* tools 调用这些函数。
...
agent-server 会话、prompt、工具循环
  ↓
文档与环境上下文：/system/prompt + 当前 full_code_path
  ↓
...
| Prompt/文档 | `core/agent-server/prompt/system/prompt` | 平台说明、SDK 说明、模式提示词、案例库、工作环境模板 |
...
- `mode/*/config.json` 与 `system_prompt.md`：不同模式可用工具和操作规则。
...
| 平台工具 | `search_tools`、`search_hub_directory`、`copy_directory`、`publish_to_hub`、`record_workspace_event`、`web_search` | 搜索复用、发布复用、记录事件、外部信息 |
...
3. 模型读 platform/SDK/case docs
4. 模型判断应该创建 Table、Form 还是 Chart
5. 模型用 write_go_file 或 search_replace_file 落盘 Go 代码
6. 模型调用 build_workspace
7. runtime 编译 Go
8. SDK 启动并 CompileAndValidate
9. SDK onAppUpdate 生成 schema diff
10. app-server 同步 function/service_tree
11. 前端刷新后可见新函数
12. 模型用 run_* 工具执行验证
13. 用户得到可运行、可复用、可继续迭代的函数资产
```
...
### 10.2 prompt/docs 与代码可能漂移

prompt 文档告诉模型怎么写，但如果 SDK 已改而文档没改，模型会继续生成旧写法。你们现在已经把文档放到 `/system/prompt`，这是对的，但还需要把文档变成版本化、可测试的资产。
...
这个思路是成立的，而且比单纯 AI 写代码更有平台价值。下一阶段最值得投入的是“契约收敛”：把组件、schema、文档、前端注册、validator 这些分散约定变成可生成、可校验、可测试的一套平台协议。
