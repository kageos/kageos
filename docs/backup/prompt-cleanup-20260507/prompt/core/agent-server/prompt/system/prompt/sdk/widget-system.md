...
```go
Title string `json:"title" gorm:"column:title" widget:"name:标题;type:input;placeholder:请输入标题" validate:"required,min=2,max=100"`
```
...
|---|---|---|---|
| 主键 ID | `int` | `ID` | 必须大写 `ID`，常配 `hide:"create,update"`；列表按 ID 筛选时写到 Table Request |
...
| 跳转链接 | `string` | `link` | 只读跳转，一般 `gorm:"-"` + `hide:"create,update"` |
...
Table 在 Request 中显式声明筛选字段，并用 `query.PageSortReq`；Model 字段不承担筛选协议。
...
| hide 值 | 含义 |
|---|---|
| `create,update` | 只在 Table 列表展示，不进入新增/编辑表单 |
| `list,update` | 只在新增表单展示 |
| `list,create` | 只在编辑表单展示 |
| `list` | 新增/编辑展示，列表不展示 |

规则：

- 系统字段如 `ID`、创建时间、更新时间、创建人通常 `hide:"create,update"`。
- 只读计算字段、link 字段、关联展示字段通常 `gorm:"-"` + `hide:"create,update"`。
- 不要写未确认的 `readonly` 之类 widget 参数来控制只读。Table 字段是否进入新增/编辑表单由 `hide` 控制；内部字段用 `widget:"-"` 隐藏。
...
```go
ObjectID int `json:"object_id" gorm:"column:object_id" widget:"name:评价对象;type:select" callback:"OnSelectFuzzy"`
