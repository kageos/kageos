...
> 身份文档包告诉模型该按什么场景工作，文档提供权威规则，tools 限制模型动作，SDK 把 Go 代码编译成平台 schema，runtime 确认新版本能启动，app-server 把 schema 沉淀为函数资产，前端和 Agent 再基于同一份 schema 渲染与调用。
...
| 平台工具 | `search_tools`、`record_workspace_event`、`web_search` | 搜索已注册函数、事件记录和外部信息；Hub 搜索/复制/发布/推送统一走 `/system/openapi/hub` |
...
- `search_tools` 主要搜内置工具和 `system` 用户下已注册函数；当前目录看环境或 `read_dir`，Hub 复用先读 `system.openapi.hub`，再调用 `/system/openapi/hub/search.form` / `copy.form`。
...
- 列表筛选：Table Request 显式字段 + Handler 手写 `Where`。
...
- Table 筛选字段写在 Request 中，Handler Build 前手写查询条件。
