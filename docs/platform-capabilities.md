...
消息能力由 `message-server` 统一承载。业务应用通过 SDK 的 `ctx.SendMessage` 发布异步消息命令，Agent 工具通过 `send_notification` 发布通知命令，最终都进入 NATS subject `message.v1.cmd.send`。
...
- 通用权限、审批、评论、收藏、外部通知渠道和备份控制面属于平台能力，不应由单个业务 App 重复实现。
