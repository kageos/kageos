...
默认输出大文件 Top 50；需要更完整的大文件清单时可以使用：

```bash
TOP_N=100 scripts/codebase-health-report.sh
```

当前脚本口径：

- 基于当前 Git 跟踪文件和未被 `.gitignore` 排除的新文件。
...
| 类别 | 文件数 | 行数 |
| --- | ---: | ---: |
| Git 跟踪和未忽略文本文件 | 1,332 | 250,405 |
| 去掉明显生成物后的手写源码 | 1,108 | 189,076 |
| 生成物或数据型文件 | 18 | 28,284 |
| 其他文档和配置文本 | 206 | 33,045 |
...
| --- | ---: | ---: |
| `web/src` | 532 | 104,856 |
| `core/agent-server` | 174 | 30,196 |
| `core/app-server` | 86 | 26,525 |
| `sdk/agent-app` | 104 | 17,360 |
| `pkg` | 121 | 13,758 |
| `web` | 32 | 12,773 |
| `core/app-storage` | 32 | 10,670 |
| `core/app-runtime` | 48 | 8,266 |
| `docs` | 25 | 5,563 |
| `core/hr-server` | 26 | 3,865 |
| `deploy` | 62 | 3,670 |
| `cmd` | 4 | 3,316 |
| `scripts` | 33 | 3,224 |
| `dto` | 24 | 2,559 |
| `core/api-gateway` | 11 | 1,606 |
...
| --- | ---: | ---: |
| `web/src` | 521 | 103,743 |
| `core/agent-server` | 108 | 16,604 |
| `sdk/agent-app` | 94 | 14,938 |
| `core/app-server` | 80 | 13,315 |
| `pkg` | 109 | 12,677 |
| `core/app-runtime` | 47 | 8,170 |
| `core/hr-server` | 26 | 3,865 |
| `cmd` | 4 | 3,316 |
| `core/app-storage` | 16 | 2,755 |
| `scripts` | 30 | 2,628 |
| `dto` | 24 | 2,559 |
| `core/api-gateway` | 8 | 1,390 |
| `deploy` | 19 | 1,263 |
| `web` | 16 | 1,132 |
...
- 大文件较多。当前 80 个手写源码文件超过 500 行，33 个超过 800 行，6 个超过 1200 行。
...
已新增 `scripts/codebase-health-report.sh`，用于定期输出：
...
这比单纯看总行数更有用，可以避免被 Swagger、lockfile、bundle 或文档误导。后续删减功能、拆大文件、清兼容层之前，优先跑这个脚本确认变化趋势。

## 建议的优化顺序

第一阶段：建立口径，不急着大拆。

- 使用 `scripts/codebase-health-report.sh` 固化代码体量统计口径。
- 排除生成物，形成每次优化前后的可比数据。
- 列出当前大文件 Top 50。
