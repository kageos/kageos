...
kageos 导出的 `capability.bundle.v1` 首先是可安装服务目录，其次才是 Hub 投稿输入。
安装内容和运营数据必须分离：kageos 只导出能够从目录本身确定的事实，价格、图片、
营销文案和审核状态由 Hub 管理。

## 顶层结构

服务目录继续使用 `capability.bundle.v1`，新增可选的 `metadata.directory`：
...
投稿者账号不写入服务目录，避免把工作空间用户、应用路径或租户信息泄露到可分发 JSON。
...
这样同一个服务目录可以在不依赖 Hub 的情况下继续安装、导入和迁移。
