...
//	- Now(-2): 2小时前
//
//	绝对日期时间（用于 gte/lte 等查询，执行模式 run_table_search 会转为时间戳）：
//	- Now(2026-02-01 13:05:05): 指定日期时间（毫秒时间戳）
//	- Now(2026-02-01): 指定日期当天 00:00:00（毫秒时间戳）
//
//	参数格式说明：
//	- 参数不需要引号，直接写在括号内即可
//	- 支持正负号：+ 表示未来，- 表示过去
...
func (t *Timestamp) WidgetLLMFacts(field *Field, opts SummaryOptions) []SemanticFact {
	facts := []SemanticFact{
		{Key: "example", Value: "1775882690000"},
	}
	if t.Format != "" {
