...
**📘 首读： [项目代码蓝图（给人和大模型的统一上下文）](docs/项目代码蓝图-给人和大模型的统一上下文.md)**  
从当前真实代码出发，统一解释平台的价值观、世界观、核心抽象、服务边界、SDK 契约、Widget 实现、工作台机制和扩展路径。  
如果旧文档之间有出入，建议先以这篇为主建立整体上下文。

**📗 如果当前目标是收口后打穿第一个垂直领域： [连锁门店运营第一阶段蓝图](docs/连锁门店运营-第一阶段蓝图.md)**  
把“为什么优先选连锁门店运营、第一阶段做什么、怎么映射到 ServiceTree / Widget / Agent / 标准 API、未来怎么扩”写成一份可直接给团队和大模型读的落地蓝图。

如果下面历史 `blueprint/` 文档在当前分支不存在或内容与代码不一致，优先以 `docs/` 下这两篇新文档和当前代码为准。
...
1. **🎯 [设计理念](blueprint/00-设计理念.md)** - **最重要**：理解项目的核心设计哲学
   - 为什么需要统一架构平台
   - 如何打破数据孤岛和智能孤岛
   - 系统特征：可编排、自描述、可移植、可监控、可复制、原子性
   - 统一架构实现：服务树、工具注册、权限系统、UI 渲染、运行时
   - **建议先阅读此文档，理解项目的设计理念后再继续**

2. **📖 [项目概述](blueprint/README.md#项目概述)** - 项目是什么、解决什么问题、核心价值
   - 如果 `blueprint/` 目录还没有文档，请先阅读本 README 的"项目愿景"和"核心特性"章节
   
3. **🏗️ [系统架构](blueprint/README.md#系统架构)** - 整体架构图、核心组件、技术栈
   - 参考本 README 的"系统架构"章节
   - 核心组件：Web 渲染引擎、App Server、App Runtime、LLM Coder

4. **💡 [核心概念](blueprint/README.md#核心概念)** - 应用、服务目录、函数、模板等核心概念
