...
export function isCacheExpired(cacheItem: CacheItem): boolean {
  const now = Date.now()
  return (now - cacheItem.timestamp) > DEPARTMENT_INFO_CACHE_CONFIG.CACHE_EXPIRY_TIME
}
...
export function waitForLoading(
  checkFn: () => boolean,
  timeout: number = DEPARTMENT_INFO_CACHE_CONFIG.API_TIMEOUT
): Promise<void> {
  return new Promise<void>((resolve) => {
    ...
    }, DEPARTMENT_INFO_CACHE_CONFIG.LOADING_CHECK_INTERVAL)
    
    // 最大超时保护
    setTimeout(() => {
      clearInterval(checkInterval)
      resolve()
    }, DEPARTMENT_INFO_CACHE_CONFIG.LOADING_MAX_TIMEOUT)
  })
}
