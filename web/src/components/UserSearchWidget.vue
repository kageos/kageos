  ...
  <div class="user-search-widget">
    ...
    >
      <div class="selected-users-list">
        <div v-for="user in selectedUsers" :key="user.username" class="selected-user-tag">
          <el-avatar v-if="user.avatar" :src="user.avatar" :size="20" class="user-avatar-small">
            {{ user.username?.[0]?.toUpperCase() || "U" }}
          </el-avatar>
          <el-avatar v-else :size="20" class="user-avatar-small">
            {{ user.username?.[0]?.toUpperCase() || "U" }}
          </el-avatar>
    ...
    >
      ...
      <el-avatar v-else :size="24" class="user-avatar-small">
        {{ selectedUser.username?.[0]?.toUpperCase() || "U" }}
      </el-avatar>
      <span class="user-display-text">
    ...
    <!-- 未选中时显示按钮 -->
    <el-button v-else @click="handleOpenDialog()">
...
// 监听值变化，加载用户信息
watch(
  () => [props.modelValue, supportsMultiple],
  () => {
    loadSelectedUsers();
  },
  { immediate: true },
