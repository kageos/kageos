...
describe('normalizeStorageFileDisplayUrl', () => {
  it('prefixes relative storage refs with the current origin', () => {
    expect(normalizeStorageFileDisplayUrl('ai-agent-os/output/preview.png')).toBe('http://localhost:3000/ai-agent-os/output/preview.png')
  })
  ...
  it('rewrites local MinIO URLs to the current origin and keeps query params', () => {
    expect(normalizeStorageFileDisplayUrl('http://localhost:9000/ai-agent-os/output/preview.png?sign=1')).toBe('http://localhost:3000/ai-agent-os/output/preview.png?sign=1')
  })
