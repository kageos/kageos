  ...
  permissions: [
    {
      validator: (rule, value, callback) => {
        ...
        const hasPermissions = Object.values(permissionGroups).some((actions) => Array.isArray(actions) && actions.length > 0)
        if (!hasPermissions) {
...
function getRolePermissions(role: Role): Record<string, string[]> {
  ...
  for (const perm of role.permissions) {
    const actions = result[perm.resource_type] ?? (result[perm.resource_type] = [])
  ...
  try {
    ...
    if (roleData.permissions && roleData.permissions.length > 0) {
      for (const perm of roleData.permissions) {
        const actions = roleForm.permissions[perm.resource_type] ?? (roleForm.permissions[perm.resource_type] = [])
        actions.push(perm.action)
      }
    }
