...
func Main(ctx context.Context, stopCh <-chan struct{}, readyCh chan<- struct{}) error {
	...
	logger.Infof(ctx, "Control-service started successfully")
	
	// ⭐ 发送就绪信号（如果提供了 readyCh）
	// 使用阻塞式发送，确保信号被接收（channel 容量为 1，不会阻塞太久）
	if readyCh != nil {
		readyCh <- struct{}{}
		logger.Infof(ctx, "Control-service 就绪信号已发送")
	}
	...
	case <-ctx.Done():
		// 上下文被取消
		logger.Infof(ctx, "Context cancelled, shutting down control-service...")
	case <-stopCh:
		// 收到停止信号
		logger.Infof(ctx, "Received stop signal, shutting down control-service...")
	}
