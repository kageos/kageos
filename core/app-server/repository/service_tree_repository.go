...
// Create 创建服务树节点（通用方法）
func (r *ServiceTreeRepository) Create(ctx context.Context, serviceTree *model.ServiceTree) error {
	return r.db.WithContext(ctx).Create(serviceTree).Error
}

// GetDocsNodesByPathPrefix 根据路径前缀获取所有 docs 类型的后代节点
func (r *ServiceTreeRepository) GetDocsNodesByPathPrefix(ctx context.Context, appID int64, parentPath string) ([]*model.ServiceTree, error) {
	prefix := strings.TrimSuffix(parentPath, "/") + "/"
	var nodes []*model.ServiceTree
	err := r.db.WithContext(ctx).Where("app_id = ? AND full_code_path LIKE ? AND type = ?",
...
func (r *ServiceTreeRepository) CreateServiceTreeWithParentPath(ctx context.Context, serviceTree *model.ServiceTree, parentFullIDPath string) error {
	// 直接创建，不再计算FullIDPath
	return r.db.WithContext(ctx).Create(serviceTree).Error
}

// CreateServiceTreeWithAppPrefix 创建带有用户应用前缀的服务目录
func (r *ServiceTreeRepository) CreateServiceTreeWithAppPrefix(ctx context.Context, serviceTree *model.ServiceTree, userAppPrefix string) error {
	// 先保存到数据库获取ID
	if err := r.db.WithContext(ctx).Create(serviceTree).Error; err != nil {
		return err
	}

	// 然后计算包含用户应用前缀的路径信息
	if err := r.calculatePathsWithAppPrefix(ctx, serviceTree, userAppPrefix); err != nil {
		return fmt.Errorf("failed to calculate paths with app prefix: %w", err)
	}

	// 更新路径信息到数据库
	return r.db.WithContext(ctx).Save(serviceTree).Error
}

// GetServiceTreeByID 根据ID获取服务目录
func (r *ServiceTreeRepository) GetServiceTreeByID(ctx context.Context, id int64) (*model.ServiceTree, error) {
	var serviceTree model.ServiceTree
	err := r.db.WithContext(ctx).Where("id = ?", id).First(&serviceTree).Error
	if err != nil {
		return nil, err
	}
...
func (r *ServiceTreeRepository) GetServiceTreesByAppID(ctx context.Context, appID int64) ([]*model.ServiceTree, error) {
	var serviceTrees []*model.ServiceTree
	err := r.db.WithContext(ctx).Where("app_id = ?", appID).Order("id ASC").Find(&serviceTrees).Error
	if err != nil {
		return nil, err
	}
...
func (r *ServiceTreeRepository) GetRootNodeByAppID(ctx context.Context, appID int64) (*model.ServiceTree, error) {
	var root model.ServiceTree
	err := r.db.WithContext(ctx).Where("app_id = ? AND ref_id = ?", appID, appID).
		Preload("App").
		First(&root).Error
	if err != nil {
		return nil, err
...
func (r *ServiceTreeRepository) GetServiceTreesByAppIDAndType(ctx context.Context, appID int64, nodeType string) ([]*model.ServiceTree, error) {
	var serviceTrees []*model.ServiceTree
...
func (r *ServiceTreeRepository) GetServiceTreeChildren(ctx context.Context, parentID int64) ([]*model.ServiceTree, error) {
	parent, err := r.GetServiceTreeByID(ctx, parentID)
	if err != nil {
		return nil, fmt.Errorf("查询父节点失败: %w", err)
	}
	return r.GetDirectChildrenByPath(ctx, parent.AppID, parent.FullCodePath)
}

// GetDirectChildrenByPath 根据路径获取直接子节点（只有下一层深度）
func (r *ServiceTreeRepository) GetDirectChildrenByPath(ctx context.Context, appID int64, parentPath string) ([]*model.ServiceTree, error) {
	prefix := strings.TrimSuffix(parentPath, "/") + "/"
	var all []*model.ServiceTree
	err := r.db.WithContext(ctx).Where("app_id = ? AND full_code_path LIKE ?", appID, prefix+"%").
		Preload("Function").
		Order("id ASC").
		Find(&all).Error
	if err != nil {
...
func (r *ServiceTreeRepository) BuildServiceTree(ctx context.Context, appID int64) ([]*model.ServiceTree, error) {
	// 获取所有服务目录
	allTrees, err := r.GetServiceTreesByAppID(ctx, appID)
	if err != nil {
		return nil, err
	}
	return r.buildTreeFromNodes(ctx, allTrees), nil
}

// BuildServiceTreeByType 根据类型构建树形结构
func (r *ServiceTreeRepository) BuildServiceTreeByType(ctx context.Context, appID int64, nodeType string) ([]*model.ServiceTree, error) {
	// 获取指定类型的服务目录
	allTrees, err := r.GetServiceTreesByAppIDAndType(ctx, appID, nodeType)
	if err != nil {
		return nil, err
	}
	return r.buildTreeFromNodes(ctx, allTrees), nil
...
func (r *ServiceTreeRepository) BuildServiceTreeByVersion(ctx context.Context, appID int64, versionNum int) ([]*model.ServiceTree, error) {
	...
	err := r.db.WithContext(ctx).Where("app_id = ? AND add_version_num <= ? AND (update_version_num = 0 OR update_version_num <= ?)",
...
func (r *ServiceTreeRepository) buildTreeFromNodes(_ context.Context, allTrees []*model.ServiceTree) []*model.ServiceTree {
...
func (r *ServiceTreeRepository) UpdateServiceTree(ctx context.Context, serviceTree *model.ServiceTree) error {
	return r.db.WithContext(ctx).Save(serviceTree).Error
}

// DeleteServiceTree 删除服务目录（级联删除子目录）
func (r *ServiceTreeRepository) DeleteServiceTree(ctx context.Context, id int64) error {
	// 先删除所有子目录
	children, err := r.GetServiceTreeChildren(ctx, id)
	if err != nil {
		return err
	}

	for _, child := range children {
		if err := r.DeleteServiceTree(ctx, child.ID); err != nil {
			return err
		}
	}
...
func (r *ServiceTreeRepository) calculatePathsWithAppPrefix(_ context.Context, serviceTree *model.ServiceTree, userAppPrefix string) error {
...
func (r *ServiceTreeRepository) GetServiceTreeByFullPath(ctx context.Context, fullPath string) (*model.ServiceTree, error) {
	fullPath = normalizeFullCodePath(fullPath)
	if fullPath == "" {
		return nil, gorm.ErrRecordNotFound
	}
	var serviceTree model.ServiceTree
	err := r.db.WithContext(ctx).Where("full_code_path = ?", fullPath).Order("id ASC").First(&serviceTree).Error
	if err != nil {
		return nil, err
	}
	return &serviceTree, nil
...
func (r *ServiceTreeRepository) GetNodeByPath(ctx context.Context, resourcePath string) (*model.ServiceTree, error) {
	return r.GetServiceTreeByFullPath(ctx, resourcePath)
}
...
func (r *ServiceTreeRepository) GetServiceTreeByFullPaths(ctx context.Context, fullPaths []string) (map[string]*model.ServiceTree, error) {
	if len(fullPaths) == 0 {
		return make(map[string]*model.ServiceTree), nil
	}

	var serviceTrees []*model.ServiceTree
	err := r.db.WithContext(ctx).Where("full_code_path IN ?", fullPaths).Order("id ASC").Find(&serviceTrees).Error
	if err != nil {
		return nil, err
	}
...
func (r *ServiceTreeRepository) CheckNameExists(ctx context.Context, parentID int64, code string, appID int64) (bool, error) {
	if parentID == 0 {
		return false, nil
	}
	parent, err := r.GetServiceTreeByID(ctx, parentID)
	...
	return r.CheckNameExistsByPath(ctx, parent.FullCodePath, code, appID)
}

// CheckNameExistsByPath 基于路径检查同级是否已存在同 code 节点
func (r *ServiceTreeRepository) CheckNameExistsByPath(ctx context.Context, parentPath string, code string, appID int64) (bool, error) {
	targetPath := strings.TrimSuffix(parentPath, "/") + "/" + code
	var count int64
	err := r.db.WithContext(ctx).Model(&model.ServiceTree{}).
...
func (r *ServiceTreeRepository) GetByID(ctx context.Context, parentId int64) (*model.ServiceTree, error) {
	var tree model.ServiceTree
	err := r.db.WithContext(ctx).Model(&model.ServiceTree{}).Where("id = ?", parentId).First(&tree).Error
	if err != nil {
		return nil, err
	}
	return &tree, nil
...
func (r *ServiceTreeRepository) GetDescendantDirectories(ctx context.Context, appID int64, parentFullCodePath string) ([]*model.ServiceTree, error) {
	// 标准化路径（确保以 / 结尾，用于前缀匹配）
...
func (r *ServiceTreeRepository) GetDescendantNodes(ctx context.Context, appID int64, parentFullCodePath string) ([]*model.ServiceTree, error) {
	normalizedPath := strings.TrimSuffix(parentFullCodePath, "/") + "/"

	var descendants []*model.ServiceTree
	err := r.db.WithContext(ctx).Where("app_id = ? AND full_code_path LIKE ?",
...
func (r *ServiceTreeRepository) SearchFunctions(ctx context.Context, currentUser, user, app, keyword, fullCodePath, templateType string, page, pageSize int) ([]*model.ServiceTree, int64, error) {
	...
	if user != "" {
		subq := r.db.WithContext(ctx).Model(&model.App{}).Select("id").Where("user = ?", user)
		if app != "" {
			subq = subq.Where("code = ?", app)
		}
		query = query.Where("service_tree.app_id IN (?)", subq)
	} else if currentUser != "" && app != "" {
		subq := r.db.WithContext(ctx).Model(&model.App{}).Select("id").Where("code = ?", app)
		query = query.Where("service_tree.app_id IN (?)", subq)
	}
	if templateType != "" {
...
func (r *ServiceTreeRepository) SearchResources(ctx context.Context, currentUser, user, app, keyword, fullCodePath string, nodeTypes []string, page, pageSize int) ([]*model.ServiceTree, int64, error) {
	query := r.db.WithContext(ctx).Model(&model.ServiceTree{}).
