...
// Table 更新日志是 MVP 主链路能力，个人用户和小团队均可查看。
type TableOperateLog struct {
	models.Base

	// 更新追溯字段：Who、When、What、Where。
	...
	RequestUser string `json:"request_user" gorm:"type:varchar(255);not null;index:idx_user;comment:请求用户（实际执行操作的用户）"`     // Who：实际执行操作的用户
	Action      string `json:"action" gorm:"type:varchar(50);not null;index:idx_action;comment:操作类型"`                     // What：OnTableAddRow、OnTableUpdateRow、OnTableDeleteRows
	IPAddress   string `json:"ip_address" gorm:"type:varchar(50);comment:IP地址"`                                           // Where：IP
	UserAgent   string `json:"user_agent" gorm:"type:varchar(500);comment:User Agent"`                                    // Where：User Agent

	// Table 特有字段
	App          string `json:"app" gorm:"type:varchar(100);not null;comment:应用名"`                                                                                                                                                // 应用名（如：demo）
	FullCodePath string `json:"full_code_path" gorm:"type:varchar(500);not null;index:idx_full_code_path;index:idx_full_code_path_row;index:idx_user_full_code_path_row;index:idx_full_code_path_created;comment:完整代码路径（与服务树对齐）"` // 完整代码路径（与服务树对齐，如：/luobei/demo/crm/crm_ticket）
