...
type WorkspacePermission struct {
	models.Base
	AppID        int64     `json:"app_id" gorm:"column:app_id;not null;index:idx_app_subject;index:idx_app_resource;comment:工作空间ID"`
	SubjectType  string    `json:"subject_type" gorm:"column:subject_type;type:varchar(20);not null;index:idx_app_subject;index:idx_subject_resource;comment:权限主体类型：user（用户）、department（组织架构）"`
	Subject      string    `json:"subject" gorm:"column:subject;type:varchar(500);not null;index:idx_app_subject;index:idx_subject_resource;comment:权限主体：用户名或组织架构路径"`
	ResourcePath string    `json:"resource_path" gorm:"column:resource_path;type:varchar(500);not null;index:idx_app_resource;index:idx_subject_resource;index:idx_resource_action;comment:资源路径"`
	...
	IsWildcard    bool      `json:"is_wildcard" gorm:"column:is_wildcard;type:tinyint(1);not null;default:0;index:idx_app_resource;comment:是否通配符路径"`
...
type PermissionRequest struct {
	...
	Subject        string     `json:"subject" gorm:"column:subject;type:varchar(500);not null;comment:权限主体"`
	ResourcePath   string     `json:"resource_path" gorm:"column:resource_path;type:varchar(500);not null;index:idx_resource_status;comment:资源路径"`
...
type PermissionGrantLog struct {
	...
	Grantee      string     `json:"grantee" gorm:"column:grantee;type:varchar(500);not null;index:idx_grantee;comment:被授权人"`
	ResourcePath string     `json:"resource_path" gorm:"column:resource_path;type:varchar(500);not null;index:idx_resource;comment:资源路径"`
	Action       string     `json:"action" gorm:"column:action;type:varchar(50);not null;comment:操作类型"`
	StartTime    time.Time  `json:"start_time" gorm:"column:start_time;type:datetime;not null;comment:权限开始时间"`
	EndTime      *time.Time `json:"end_time" gorm:"column:end_time;type:datetime;comment:权限结束时间（NULL 表示永久）"`
...
type ApprovalPolicy struct {
	...
	ResourcePath     string    `json:"resource_path" gorm:"column:resource_path;type:varchar(500);not null;index:idx_resource_policy;index:idx_app_resource;comment:目录路径"`
	PolicyExpression string    `json:"policy_expression" gorm:"column:policy_expression;type:varchar(500);not null;comment:审批策略表达式"`
	Description      string    `json:"description" gorm:"column:description;type:varchar(500);comment:策略描述"`
