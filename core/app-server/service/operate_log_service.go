...
func (s *OperateLogService) GetOperateLogs(ctx context.Context, req *dto.GetOperateLogsReq) (*dto.GetOperateLogsResp, error) {
	...
	for i, log := range logs {
		logItems[i] = dto.OperateLogItem{
			...
			WorkspaceSessionID:    log.WorkspaceSessionID,
			WorkspaceSessionTitle: log.WorkspaceSessionTitle,
			WorkspaceRole:         log.WorkspaceRole,
			InitiatorUser:         log.InitiatorUser,
			WorkspaceMessageID:    log.WorkspaceMessageID,
