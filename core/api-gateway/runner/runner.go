...
import (
	...
	"github.com/kageos/kageos/core/api-gateway/server"
	"github.com/kageos/kageos/pkg/config"
	"github.com/kageos/kageos/pkg/logger"

	// 导入网关 API 以确保 swag 扫描到（独立启动时）
	_ "github.com/kageos/kageos/core/api-gateway/api/v1"
)
...
func Main(ctx context.Context, stopCh <-chan struct{}, readyCh chan<- struct{}) error {
	...
	logger.Infof(ctx, "Api-gateway started successfully")

	// ⭐ 发送就绪信号（如果提供了 readyCh）
	// 使用阻塞式发送，确保信号被接收（channel 容量为 1，不会阻塞太久）
	if readyCh != nil {
		readyCh <- struct{}{}
		logger.Infof(ctx, "Api-gateway 就绪信号已发送")
	}
	...
	case <-ctx.Done():
		// 上下文被取消
		logger.Infof(ctx, "Context cancelled, shutting down api-gateway...")
	case <-stopCh:
		// 收到停止信号
		logger.Infof(ctx, "Received stop signal, shutting down api-gateway...")
	}
