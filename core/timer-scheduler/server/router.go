...
import (
	...
	timerservice "github.com/ai-agent-os/ai-agent-os/core/timer-scheduler/service"
...
func NewRouter(service *timerservice.Service) *gin.Engine {
	...
	api.POST("/events/execution_requested", publishExecutionRequested(service))
...
func publishExecutionRequested(service *timerservice.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		var req scheduledsdk.ExecutionRequestedEvent
		if !bindJSON(c, &req) {
			return
		}
		writeResult(c, gin.H{"ok": true}, service.PublishExecutionRequested(c.Request.Context(), req))
	}
}
