...
type WorkspaceMode struct {
	...
	ToolNames            string `gorm:"type:varchar(1024);comment:该模式启用的工具名，分号分隔" json:"tool_names"` // 如 read_file;read_dir;write_doc
...
func InitWorkspaceModes(db *gorm.DB) error {
	...
	for _, code := range codes {
		...
		case "dev":
			...
			m.SetToolNames([]string{"read_file", "read_dir", "generate_code", "write_package_code", "write_doc", "create_directory", "get_current_time"})
		case "modify":
			m.Name = "修改模式"
			m.Description = "对已有应用进行修改（代码/配置）"
			m.SystemPromptFragment = "当前为修改模式，请协助用户修改已有代码或配置。"
			m.SetToolNames([]string{"read_file", "read_dir", "write_package_code", "write_doc", "create_directory", "get_current_time"})
		case "execute":
			m.Name = "执行模式"
			m.Description = "操作已生成应用（查数据、分析等）"
			m.SystemPromptFragment = "当前为执行模式，请协助用户查看数据、分析结果等。"
			m.SetToolNames([]string{"read_file", "read_dir", "get_current_time"})
		}
