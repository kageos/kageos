...
// ListWorkspaceSessions 按执行目录、具体资源和会话来源筛选。
// 第二个 OR 分支只用于兼容历史上把资源路径写进 full_code_path 的数据，不会改写历史记录。
func (r *ChatSessionRepository) ListWorkspaceSessions(ctx context.Context, opts WorkspaceSessionListOptions) ([]*model.AgentChatSession, int64, error) {
	...
	if opts.ResourceFullCodePath != "" {
		query = query.Where(
			...
			opts.FullCodePath, opts.ResourceFullCodePath, opts.ResourceFullCodePath,
		)
	} else {
	...
	if err := query.Offset(opts.Offset).Limit(opts.Limit).Order("updated_at DESC").Find(&sessions).Error; err != nil {
		return nil, 0, err
	}
	return sessions, total, nil
}

func (r *ChatSessionRepository) ListWorkspaceAutomationAgents(ctx context.Context, fullCodePath, resourceFullCodePath, user string) ([]*WorkspaceAutomationAgent, error) {
	...
	if resourceFullCodePath != "" {
		query = query.Where(
			"((full_code_path = ? AND resource_full_code_path = ?) OR (full_code_path = ? AND (resource_full_code_path IS NULL OR resource_full_code_path = '')))",
			fullCodePath, resourceFullCodePath, resourceFullCodePath,
		)
	} else {
	...
	if err := query.Group("automation_task_id").Order("MAX(id) DESC").Scan(&agents).Error; err != nil {
		return nil, err
	}
	return agents, nil
}
