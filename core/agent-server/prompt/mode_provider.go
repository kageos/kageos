...
const allInOneSystemPromptPath = "/system/prompt/doc/workbench-all-in-one-system-prompt"
...
func init() {
	for _, code := range []string{"qa", "dev", "modify", "execute", "agent"} {
...
func modeSystemPromptAppendFiles(code string, configured []string) []string {
	...
	if shouldAppendAllInOneSystemPrompt(code) && !containsString(out, allInOneSystemPromptPath) {
		out = append(out, allInOneSystemPromptPath)
	}
	return out
}

func shouldAppendAllInOneSystemPrompt(code string) bool {
	switch strings.TrimSpace(code) {
	case "dev", "agent", "modify":
		return true
	default:
		return false
	}
