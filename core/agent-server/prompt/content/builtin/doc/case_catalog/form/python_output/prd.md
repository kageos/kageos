...
代码随本案例一起提供；read_doc 本案例路径 **`/builtin/doc/case_catalog/form/python_output`**（同步 case_catalog 后）即获得 PRD 与代码，无需再调用 read_go_file。
...
// 大模型请先 read_doc /builtin/doc/case_catalog/form/python_output。
...
ax.text(0.5, 0.12, 'read_doc: /builtin/doc/case_catalog/form/python_output', ha='center', va='center', fontsize=8, color='#666')
...
output_json({
    "info": "本图由 matplotlib 在本应用容器内写入 Go 指定的绝对路径；Go 用 ResponseFiles 下发。勿用相对路径在 Go/Python 间互传 cwd 不同。详阅 read_doc /builtin/doc/case_catalog/form/python_output。"
})
`
}
...
var sandboxFileOutDemoTemplate = &app.FormTemplate{
	BaseConfig: app.BaseConfig{
		...
		Desc:     "演示：同机 Python 按 **绝对路径** 落盘，Go **ResponseFiles** 下发附件。须 defer executor.Close()。工作台 run_official_python 无法下发附件；大模型请先 read_doc /builtin/doc/case_catalog/form/python_output。",
