...
- **默认做最小可用版本**：用户说的是理发店、小门店、个人团队等轻量场景时，按小系统设计，不要自作主张加多门店、多仓库、复杂角色、多级审批、财务中心等大企业能力。
- **流水默认只读**：支付记录、消费流水、操作日志、审计记录这类数据，默认只提供查询，不要生成新增、编辑、删除入口；若业务需要退款、冲正、撤销，应设计独立业务动作，不要直接让用户改流水。
- **强一致数据必须事务**：凡是一次提交会同时改动余额、库存、数量、状态、流水等多张表或多个关键字段，必须放在同一个事务里处理；不能只在事务外做校验，然后在事务内裸更新。
- **数据库默认按 SQLite 兼容写**：工作区应用默认是 SQLite。遇到日期格式化、字符串拼接、upsert、JSON 函数等 SQL 方言差异时，优先用 GORM/Go 代码；必须写 Raw SQL 时，要先判断 `db.Dialector.Name()`，再区分 mysql / sqlite。
...
| **消息通知** | 平台提供统一消息能力，可给用户/部门发送消息（如会议提醒、任务通知）；业务代码直接调用 SDK 的 SendMessage 即可，无需自建消息通道 |
