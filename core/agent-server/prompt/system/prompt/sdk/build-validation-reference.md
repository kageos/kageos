...
2. SDK schema 错误：修 route、Template、widget、validate、search、Response 类型。
...
- `<type> has no field or method <method>`：代码把 SDK 类型当成标准库类型或旧示例类型使用。读取该类型定义和方法集后再改。
- schema validation failed：先看 router、字段名、widget、search、validate、Request/Model 是否冲突，不要只修第一条错误。
...
- `req.GetPage undefined` / `unknown field Total/DataList in query.PaginatedTable`：不要手写旧分页 API。默认用 `AutoSearchFilterPaged`；手工构造时用 `Items`、`CurrentPage`、`TotalCount`、`TotalPages`、`PageSize`。
- `types.Time has no field or method Format` 或 `Time.Format undefined`：使用 `t.Time().Format(...)`、`t.Time().After(...)`、`t.Time().Before(...)`。
- `unsupported widget type` / `unsupported widget tag` / `invalid tag format`：widget 只能使用 `widget-system` 中确认过的类型和 key。文件上传是 `type:files`，只读展示用 `display:"scenes:list"` 或 `widget:"-"`，不要编造前端习惯参数。
- `number widget requires integer Go type`：`type:number` 只用于整数；`float64` 的金额、均值、评分、比例用 `type:float`。
- `req.X undefined ... Req has no field or method X`：删除 Request 冲突字段后，Handler 里的手写 `req.X` 筛选也要同步删除；主表字段筛选靠 Model `search` 标签。
- `does not implement chart.Charter`：传给 `resp.Chart(...)` 的必须是 SDK chart 包里的具体图表对象，不是自定义响应结构体；附加指标放 `Metadata`，多图拆多路由。
- `table request field ... conflicts with table model field ...`：Request 自定义搜索字段和 Model 字段 code 重复。`gorm:"-"` 计算/列表展示字段也算 Model 字段，不能和 Request 重名；主表字段搜索写在 Model 的 `search` 标签上，计算字段筛选用 `xxx_filter` 这类独立 code。
...
- `audit field "id"/"created_at"/"updated_at"`：系统字段必须使用规定的 widget、search、display 和 gorm tag；不要省略这些 tag，也不要把 `DeletedAt` 暴露到 schema。
