...
1. Model：落库字段、前端展示字段、搜索和校验。
2. Request：列表搜索条件，嵌入 `query.SearchFilterPageReq`。
3. TableTemplate：声明 Request、Response、CreateTables、AutoCrudTable 和写操作回调。
4. List 函数：查询数据库并 `resp.Table(...).AutoSearchFilterPaged(...).Build()`。
5. `init()`：注册路由。
...
func TicketList(ctx *app.Context, resp response.Response) error {
    ...
    return resp.Table(&lists).AutoSearchFilterPaged(db, &Ticket{}, &req.SearchFilterPageReq).Build()
}
...
- 只读表也建议显式配置 `AutoCrudTable`，它决定前端列表按哪个 Model 渲染字段、搜索、分页和 schema。
...
var PaymentRecordListTemplate = &app.TableTemplate{
    BaseConfig: app.BaseConfig{
        ...
        Response:     query.PaginatedTable[[]*PaymentRecord]{},
...
1. `Request` 嵌入 `query.SearchFilterPageReq`，并用 `widget:"-"` 隐藏分页字段。
2. `queryDB := ctx.GetGormDB().Model(&Model{})`。
3. Build 前做手写 `Where`、`Preload`、排序等。
4. 调 `resp.Table(&lists).AutoSearchFilterPaged(queryDB, &Model{}, &req.SearchFilterPageReq).Build()`。
5. Build 后填充不落库字段，例如 link、计算状态、关联展示名。

外表筛选和计算字段筛选：

- 如果搜索条件不是主表数据库列，放在 Request 结构体中。
- 在 Build 前手写查询逻辑。
- 不要指望 `search` 自动处理复杂关联字段。
...
`query.SearchFilterPageReq` 负责接收前端基于 Model `search` 标签生成的通用搜索、排序和分页。只要搜索条件不是主表字段，就在 Request 中增加自定义参数。
...
func BookingList(ctx *app.Context, resp response.Response) error {
    ...
    var lists []*Booking
    if err := resp.Table(&lists).AutoSearchFilterPaged(queryDB, &Booking{}, &req.SearchFilterPageReq).Build(); err != nil {
        return err
    }
    return nil
}
...
- `query.SearchFilterPageReq` 要匿名嵌入并加 `widget:"-"`，不要把分页字段暴露成普通表单字段。
- 主表已有字段不要在 Request 中重复定义同名 `json` / `form` code；例如 Model 已有 `item_id`，Request 里不要再写 `ItemID json:"item_id"`，否则 table schema 会报字段冲突。
- `gorm:"-"` 的列表计算/展示字段也算 Model 字段，同样不能和 Request 重名；如果 Model 展示字段是 `status json:"status"`，Request 计算筛选字段应命名为 `StatusFilter json:"status_filter"`。
- 自定义 Request 字段的 `json` 名不要和任何 Model 字段冲突。
- 主表字段搜索优先写在 Model `search` 标签中，不要重复手写。
- 关联字段、计算字段、特殊权限过滤、跨表筛选，放在 Build 前手写 `Where` / `Joins`。
- 自定义搜索字段要有 `widget`，否则前端不会渲染搜索控件。
- 默认使用 `resp.Table(&lists).AutoSearchFilterPaged(queryDB, &Model{}, &req.SearchFilterPageReq).Build()`；不要调用不存在的 `req.GetPage()` / `req.GetPageSize()`，也不要用 `Total` / `DataList` 手工构造 `query.PaginatedTable`。
- 如果确实要手工构造 `query.PaginatedTable`，字段名是 `Items`、`CurrentPage`、`TotalCount`、`TotalPages`、`PageSize`。
...
if err := resp.Table(&records).AutoSearchFilterPaged(queryDB, &VoteRecord{}, &req.SearchFilterPageReq).Build(); err != nil {
    return err
}
...
`display` 决定字段出现在哪个前端场景：

- `display:"scenes:list"`：只在 Element 表格列中展示，不进入新增/编辑表单。ID、创建时间、更新时间、创建人、link、计算字段、关联名称都应这样写。
- `display:"scenes:create"`：只在新增表单中出现，例如创建投票主题时输入选项子表。
- `display:"scenes:update"`：只在编辑表单中出现。
- 不写 `display`：默认进入列表和新增/编辑，只有用户真正需要看和填的业务字段才这么做。
- `widget:"-"`：完全不进入前端 schema，适合 `DeletedAt`、GORM 关联对象、内部中间字段。

典型字段：

```go
TopicTitle string `json:"topic_title" gorm:"-" widget:"name:投票主题;type:text" display:"scenes:list"`
ActionLink string `json:"action_link" gorm:"-" widget:"name:操作;type:link;target:_blank" display:"scenes:list"`
```
...
- 跳 Table：参数使用目标 Table 的 Model 或 Request，前端会转成搜索筛选。
- 跳 Form：参数使用目标 Form Request，用于预填。
- 跳 Chart：参数使用目标 Chart Request，用于筛选图表。
- link 字段通常 `gorm:"-"` + `display:"scenes:list"`。
...
```go
ObjectID int `json:"object_id" gorm:"column:object_id" widget:"name:评价对象;type:select" search:"in" validate:"required" callback:"OnSelectFuzzy"`
ObjectName string `json:"object_name" gorm:"-" widget:"name:评价对象;type:text" display:"scenes:list"`
```

同时在 `TableTemplate.BaseConfig.OnSelectFuzzyMap` 中注册 `"object_id"`。回调要支持关键字搜索，也要支持 by_value / by_values 回显。
...
- 计算字段、关联名称、状态文本通常 `gorm:"-"` + `display:"scenes:list"`。
...
- Model 已有字段又在 Request 里定义同名搜索字段，导致 table request fields conflicts；`gorm:"-"` 计算/展示字段也不能重复，筛选字段用 `xxx_filter`。
