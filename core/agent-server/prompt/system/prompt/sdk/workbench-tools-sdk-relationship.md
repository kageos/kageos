...
这份文档用于给工作台大模型快速建立全局心智模型：工作台不是普通聊天框，tools 不是普通函数列表，SDK 也不是普通 Go 工具库。它们共同组成一条“意图识别 -> Skills 路由 -> 读取权威文档 -> 调用受控 tools -> 生成代码 -> 编译启动 -> schema 注册 -> 前端渲染 -> Agent 调用 -> 资产沉淀”的闭环。

## 一句话

> Skills 告诉模型该按什么场景工作，文档提供权威规则，tools 限制模型动作，SDK 把 Go 代码编译成平台 schema，runtime 确认新版本能启动，app-server 把 schema 沉淀为函数资产，前端和 Agent 再基于同一份 schema 渲染与调用。
...
agent-server：LLM 会话、prompt、tool loop
  ↓
Skills：按意图选择 SOP / SDK / OpenAPI / tools
  ↓
...
| Skills | `core/agent-server/skills` | 按场景提供 SOP、必读文档、工具建议和验收清单 |
...
## 4. Skills、文档和 prompt 的作用

Skills、文档和 prompt 分层工作：

- **Prompt**：只放身份、模式边界、环境信息和极简总规则。
- **Skills**：按用户意图选择 SOP，例如 `sop.create-project`、`sdk.widget-selection`、`system.openapi.message`、`system.tools.pdf`。
- **Docs**：权威参考，承载 SDK 细节、平台边界、案例代码和长规则。

不要把 SDK 长文档整体塞进 skill；skill 只写任务导航、必读文档、关键坑和验收清单。
...
- `/system/prompt/sdk/widget-reference`：字段建模和 widget 选择。
- `/system/prompt/sdk/form-table-chart-reference`：Form/Table/Chart 写法短参考。
- `/system/prompt/sdk/build-validation-reference`：build 与启动期校验链路。
- `/system/prompt/sdk/platform-api-reference`：`ctx.APICall` 和 `ctx.SendMessage`。
...
- Table request 字段不能和 AutoCrudTable / Response 表字段 `json` code 重名。
- `search` 只允许前端 Table 搜索栏真实支持的 `eq`、`like`、`in`、`contains`、`gte,lte`。
