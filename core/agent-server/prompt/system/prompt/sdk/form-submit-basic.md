...
跳 Table 时，参数会按目标 Table 的 search 标签转成查询条件；跳 Form 或 Chart 时，参数会按目标 Request 转成表单参数。
...
### display、search 和 Form 的边界

Form Request/Response 通常不需要 `display:"scenes:list"`，因为它不是 Table 列表。只有当同一个结构体也作为 Table Model 使用时，才需要严格写 `display`。

- `display:"scenes:list"`：只给 Table 列表字段、只读计算字段、link 字段使用。
- `search`：只给 Table 搜索字段使用；单 Form 不嵌入 `query.SearchFilterPageReq`。
...
- Table Model 中系统字段、计算字段、关联展示字段要用 `display:"scenes:list"` 控制展示。
