...
本文档用于所有 Form/Table/Chart 场景。它只覆盖横切运行能力：消息通知、平台 API、当前用户上下文、权限和审批边界、定时任务、Table 回调高级能力、事务和副作用、日志错误处理、Python 运行时、构建校验。具体 Form/Table/Chart 写法仍以对应场景包为准。
...
Hub、权限、审批、审计、操作日志、消息等平台领域优先读对应 `system.openapi.*` skill。
...
不要把创建人、提交人、操作人作为普通 Request 字段让用户填写；需要展示时放在 Table Model 中，通常 `display:"scenes:list"`。
...
- `BindUpdates(&target)` 可把原始更新值绑定到结构体；未更新字段为零值，需要旧数据就查库或读 `GetOldValues()`。
...
1. `build_workspace`。
2. 启动期 `CompileAndValidate()` 会检查路由后缀、Template 类型、schema、widget、search、validate。
3. 根据函数类型继续验证：
