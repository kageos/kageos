...
- 导入历史按文件名、状态、导入人搜索：主表字段写 `search`，复杂统计字段用自定义参数。

写法：Request 嵌入 `query.SearchFilterPageReq`；标准外键筛选优先放在 Table Model 上，用 `search:"in"` + `callback:"OnSelectFuzzy"`。Table 新增/编辑也需要外键选择时，外键字段必须在 Table Model 上是 `type:select` / `type:multiselect`，并在同一个 `TableTemplate.BaseConfig.OnSelectFuzzyMap` 注册同名 key。不要只把外键字段放进 Request；Request 是搜索入参，不会给 `AutoCrudTable` 的新增/编辑字段提供回调。只有按外表非 ID 字段、计算字段、行业/标签这类复杂条件筛选时，才在 Request 增加自定义字段，Build 前手写 `Where` / `Joins`，再交给 `AutoSearchFilterPaged`。Request 字段 code 不能和 Model 任意字段重复，`gorm:"-"` 的计算展示字段也算；例如 Model 展示 `status` 时，Request 筛选用 `status_filter`。
...
if err := resp.Table(&records).AutoSearchFilterPaged(queryDB, &EvaluationRecord{}, &req.SearchFilterPageReq).Build(); err != nil {
    return err
}
for i := range records {
...
组合场景更需要 `display` 控制前端：

- 对象表：ID、创建时间、评价次数、平均分、提交评价 link 用 `display:"scenes:list"`。
- 评价 Form：Request 只放用户要填的对象、评分、内容、附件，不放列表字段。
- 记录表：评价内容、评分、提交人、状态、管理员回复可列表展示；GORM 关联对象用 `widget:"-"`。
- 只读计算字段、关联名称、link 都必须 `gorm:"-"` + `display:"scenes:list"`。
...
Table 记录搜索也可以用 OnSelectFuzzy。例如评价记录表中：
...
注意：搜索回调和提交回调的业务过滤可能不同。`evaluation_submit.form` 只返回开放评价的对象；`evaluation_record_list.table` 查询历史记录时应返回所有对象，包括已关闭对象，否则历史记录无法按已关闭对象筛选。
