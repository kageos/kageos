...
6. 分阶段实现：先 Table 主数据，再 Form 动作，再 Chart 统计，每阶段 build 和验证。
...
- 主表字段：在 Model 上写 `search:"like"`、`search:"in"`、`search:"gte,lte"` 等。
- 关联字段：在 Request 中新增自定义搜索参数，Build 前用 `Joins` / `Where`。
- 统计缓存字段：如果是 `gorm:"-"` 计算字段，不能靠 `search` 自动查，必须在 Request 中自定义并手写 SQL；Request 字段 code 不能和 Model 的计算展示字段重复，建议命名为 `xxx_filter`。
...
主表已有字段不要在 Request 中重复定义同名 `json` / `form` code；`gorm:"-"` 的列表计算/展示字段也算 Model 字段，不能重复。例如记录 Model 已有 `item_id` 并带 `search:"eq"`，Request 里不要再写 `ItemID json:"item_id"`；如果 Model 已有计算展示字段 `status json:"status"`，Request 筛选字段要写成 `StatusFilter json:"status_filter"`。需要按外键搜索时，优先在 Model 外键字段上配置 `widget:"type:select"`、`search:"eq"` 或 `search:"in"`、`callback:"OnSelectFuzzy"`，并在 `OnSelectFuzzyMap` 注册该字段。
...
复杂系统字段多，必须用 display 控制界面复杂度：

- 系统字段、link、计算字段、关联展示名：`display:"scenes:list"`。
- Form 专用明细输入，例如收银商品明细子表：放在 Form Request，不要塞进 Table 列表。
- Table 新增时才需要的嵌套子表：可用 `display:"scenes:create"`。
- GORM 关联对象、内部中间字段：`widget:"-"`。

没有 display 规划时，前端会把大量不该编辑的字段暴露在新增/编辑表单里。
...
统一用 `ctx.BuildFunctionUrlWithText`。跳 Table 时参数会按目标 Table 搜索规则转换；跳 Form/Chart 时参数会按目标 Request 预填。

### Form 动态选择和事务

Form 中选择商品、会员、投票主题、评价对象、客户时，使用 OnSelectFuzzy。Table 搜索区按外键筛选记录时也优先使用 OnSelectFuzzy，例如支付流水按会员、评价记录按评价对象筛选；前端显示业务名，实际提交 ID。依赖其他字段时用 `depend_on`。
...
Chart 的 Request 是筛选条件，不嵌入 `query.SearchFilterPageReq`：
...
- `payment_record_list.table` 仍然建议显式配置 `AutoCrudTable`，让前端按支付流水 Model 渲染列表、搜索和分页。
...
1. 建主数据 Table，`build_workspace`，`run_table_search`。
2. 建动作 Form，`build_workspace`，`run_form_submit`。
3. 建统计 Chart，`build_workspace`，`run_chart_query`。
4. 补 link 和联动，重新验证核心路径。
...
- 自定义 Request 字段和 Model 字段使用相同 `json` code，导致 table request fields conflicts；`gorm:"-"` 计算展示字段同样不能重复，筛选字段用 `xxx_filter`。
