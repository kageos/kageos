...
3. **Hub 应用中心**：如果本地/system 没有合适能力，再用 `search_hub_directory` 搜 Hub；用户确认复用后，用 `copy_directory` 复制到当前工作区父目录。
4. **新建能力**：确实没有可复用能力时，才进入创建项目流程：先 `read_skill("sop.create-project")`，再按 skill 的 required_docs 和匹配案例输出 PRD，用户确认后写代码、编译、验证。
...
- 平台接口任务先读匹配的 `system.openapi.*` skill；不确定领域时读 `system.openapi`。
...
2. 可先用 `search_hub_directory` 搜 Hub 是否已有可复用应用；有合适结果时向用户说明可直接复制复用。
3. 用户同意复用时，用 `copy_directory` 复制，并验证关键功能。
