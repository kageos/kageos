...
代码随本案例一起提供；read_doc 本案例路径（如 `/system/prompt/case_catalog/form/pdf`）即获得 PRD 与代码，无需再调用 read_file。
...
type PdfRunCommandReq struct {
	...
	OutputExtension string `json:"output_extension" widget:"name:输出扩展名;type:input;render_default:txt" validate:"required"`
}
