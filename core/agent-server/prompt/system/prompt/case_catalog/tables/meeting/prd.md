...
| 设备配置 | 多行文本 | ✗   | —      | 设备说明 |
| 位置信息 | 文本输入 | ✓   | —      | 2–100 字 |
| 状态     | 下拉选择 | ✓   | 可用   | 可用/维护中/停用 |

**列表模式**

| ID | 创建时间 | 更新时间 | 创建人 | 会议室名称 | 会议室类型 | 容纳人数 | 设备配置 | 位置信息 | 状态 |
|----|----------|----------|--------|------------|------------|----------|----------|----------|------|
| 1 | 2025-01-15 09:00 | 2025-01-15 09:00 | — | A栋201 | 小型 | 8 | 投影、白板 | A栋2楼 | 可用 |
| 2 | 2025-01-15 09:00 | 2025-01-16 10:00 | — | B栋大会议室 | 大型 | 50 | 投影、音响、视频 | B栋1楼 | 可用 |
...
**列表模式**（可筛：会议室名称、预约状态；会议室名称、预约状态为仅列表展示/计算字段）

| ID | 创建时间 | 更新时间 | 创建人 | 会议室 | 预约人 | 会议主题 | 会议描述 | 开始时间 | 结束时间 | 参会人数 | 备注 | 会议室名称 | 预约状态 | 操作 |
|----|----------|----------|--------|--------|--------|----------|----------|----------|----------|----------|------|------------|----------|------|
| 1 | 2025-01-20 09:00 | 2025-01-20 09:00 | 张三 | A栋201 | 张三 | 产品需求评审 | 评审Q1需求 | 2025-01-22 14:00 | 2025-01-22 16:00 | 6 | — | A栋201 | 待开始 | 删除、查看会议室详情 |
...
type MeetingRoom struct {
	ID        int            `json:"id" gorm:"primaryKey;autoIncrement;column:id" widget:"name:会议室ID;type:ID" display:"scenes:list" search:"eq"` // 前端仅在列表展示，不进入新增/编辑表单。
...
type MeetingRoomBooking struct {
	...
	Booker        string `json:"booker" gorm:"column:booker;comment:预约人" widget:"name:预约人;type:user;render_default:Me()" search:"in" validate:"required"`
