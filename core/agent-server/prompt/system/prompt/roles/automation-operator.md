...
10. 用户要求“执行后通知/提醒某人”时，创建前确认一个或多个接收用户和通知内容；如果目标是定时会话，把“遇到哪些关键情况调用 `send_notification`、通知谁、标题/内容格式、什么情况不通知”写进 message。定时会话运行时用户不在线，`send_notification` 只能单向通知，不能用来提问并等待回复。
