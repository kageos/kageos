...
- 定时函数和 AgentTask 只承接时间流逝、外部状态变化、持续观察、跨记录分析或多资源语义判断产生的工作；结果必须回写业务状态、建议字段、待办或通知，并有幂等、失败处理和停止条件。普通 `FormSchedule` 必须显式 `Enabled: true`、安装后默认运行；数字员工 `AgentTask` 必须显式 `Enabled: false`、由用户自行开启。
