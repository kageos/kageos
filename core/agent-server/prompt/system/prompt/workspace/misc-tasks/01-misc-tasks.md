...
核心原则：**先读本文档，再看环境再搜，按「用途」选类型（form/table/chart），没有则问再建**。

**文档闸门**：处理杂活前必须已读本文档。未读本文档时，不要先调用 `search_tools`、`run_form_submit`、`run_table_*`、`run_chart_query`。
...
| **table** | **管理一批记录**：增删改查「行数据」，有 id、多行、可筛选。 | 查列表（run_table_search）、新增行（run_table_create）、批量导入（run_table_batch_create）、改行（run_table_update）、删行（run_table_delete）。数据持久在库里。 | 问卷列表、工单、客户表、订单、任何「要查列表 / 加一条 / 批量导入 / 改一条 / 删一条」的业务。 | `run_table_search` / `run_table_create` / `run_table_batch_create` / `run_table_update` / `run_table_delete` |
...
根据环境信息或 `read_dir()` 看当前目录下是否已有与任务相关的能力（如画图、转换相关）。有则先确认参数结构，再执行；没有则进入下一步。
...
若返回中有**已注册函数**，根据类型选用工具执行（form 用 `run_form_submit`、table 用 `run_table_search` 等）。不必额外询问用户是否调用现成能力，但必须先确认参数结构。
...
若未匹配到可用函数：
- **若需求是纯展示、无后台、数据一次性**（如数据看板、年度报告、会议展示页）：先 `search_tools("html|网页|页面", "form")` 试一次，有 html_generate 等则按字段摘要确认参数后调用 `run_form_submit`。
- 若仍无匹配：
  - 禁止直接创建。
...
**表单（run_form_submit）的 request 与表格（run_table_create、run_table_batch_create、run_table_update）的 model** 里凡是 `widget.type === "files"` 的字段，传参时须为**字符串文件引用**，格式为 `bucket/object_key`；多文件用英文逗号分隔。
...
- **【已注册函数】**：仅限 **system 用户下**已注册的表单/表格/图表函数；含一句统一调用方式说明；每条为 name、full_code_path、已使用 N 次（若有）、description、type、字段摘要；需要原始 schema 时传 `schema_output="json"` 或 `"both"`。
