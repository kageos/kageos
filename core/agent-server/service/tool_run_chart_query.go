...
var runChartQueryToolDef = toolDefinition[runChartQueryArgs](
	"run_chart_query",
	"执行工作区内 Chart 查询接口，返回图表数据。full_code_path 为图表函数路径，如 /luobei/myapp/charts/sales。图表查询参数不固定，由具体 Chart 的 handler 定义（如 year、month、dimension 等），请用 read_go_file 查看对应 .go 的 Req 结构。传 url_query 为完整查询串（如 year=2024&month=1），不传则无额外参数。",
)
