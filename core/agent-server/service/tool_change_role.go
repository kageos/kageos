...
func (t *ChangeRoleTool) Execute(ctx context.Context, call ToolCall) ToolResult {
	...
	if strings.TrimSpace(args.TargetRole) != "" && !isKnownWorkspaceRole(args.TargetRole) {
		return toolResult("target_role 不支持: "+strings.TrimSpace(args.TargetRole)+"。请使用标准角色 ID：product_manager、app_developer、maintenance_engineer、app_operator、qa_engineer、build_engineer、data_operator、workflow_engineer、scheduler_engineer、platform_engineer、reviewer。", true)
	}
