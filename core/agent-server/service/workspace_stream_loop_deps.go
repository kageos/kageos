...
func (d *workspaceStreamLoopDeps) RequestContextReduction(ctx context.Context, reason string) bool {
	...
	// Automatic context reduction is disabled. Retrying with the same complete
	// history would only repeat the provider error, while silently dropping or
	// summarizing history would break conversation continuity.
	return false
}
