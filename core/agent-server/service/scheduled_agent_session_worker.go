...
const scheduledAgentUnattendedPrefix = "【定时会话执行约束】本次会话由定时任务自动触发，但当前目标不是创建或管理定时任务，而是执行后面的会话消息。请先选择能完成该业务执行的角色；需要调用已有 Form/Table/Chart 或连接器时，通常应进入 app_operator。执行过程中用户不在线、无法回答问题或确认操作；不要向用户提问，不要等待用户补充信息，不要把下一步停在“请确认/请提供”。如果创建时的信息不足以安全执行，按已知上下文完成可安全完成的部分，并在结果中明确记录缺失信息、未执行的动作和原因；涉及高风险写入且缺少必要确认时应跳过该动作并说明原因。"
...
func scheduledAgentSessionWorkspaceRequest(event scheduledsdk.ExecutionRequestedEvent) (*dto.WorkspaceChatReq, scheduledAgentSessionPayload, error) {
	...
	content := scheduledAgentSessionMessageContent(payload)
...
func scheduledAgentSessionMessageContent(payload scheduledAgentSessionPayload) string {
	...
	return scheduledAgentUnattendedPrefix + directoryHint + "\n\n" + content
}
