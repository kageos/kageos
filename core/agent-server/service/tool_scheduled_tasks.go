...
type createScheduledAgentTaskArgs struct {
	...
	Message            string `json:"message" schema_desc:"到点后直接发送给工作台会话的完整用户消息。必须像无人值守运行手册一样写清：任务性质、长期目标、绑定目录/资源、可用的其他目录/函数/连接器、预期使用工具清单（如 change_role/search/read_dir/web_search/run_table_search/run_table_create/run_table_update/run_form_submit/run_chart_query/send_notification）、执行步骤、按业务场景裁剪的质量控制策略、失败处理、输出格式、通知规则。定时会话运行时会注入任务创建人/请求用户作为默认通知对象；需要通知创建人、当前用户或“我”时，message 要写明调用 send_notification 必须显式传 to_users 为创建人 username；首次基准记录、无变化结果、普通状态报告默认不通知。运行时用户不在线，不能写“到时候问我/等待确认”。如果只是固定调用一个函数，请改用 create_scheduled_function_task"`
	ScheduleType       string `json:"schedule_type" schema_desc:"计划类型，可不传。只填 run_at 会自动当一次性任务；只填 cron_expr 会自动当 cron 周期任务；只填 interval_seconds 会自动当每隔 N 秒任务" schema_enum:"atime,cron,every"`
	RunAt              string `json:"run_at" schema_desc:"一次性任务的执行时间，例如 2026-06-10 09:30:00 或 RFC3339；无时区时按 timezone 解析"`
	CronExpr           string `json:"cron_expr" schema_desc:"cron 周期表达式，例如每分钟 * * * * *，每天 9 点 0 9 * * *"`
	IntervalSeconds    int64  `json:"interval_seconds" schema_desc:"每隔多少秒执行一次，例如 60 表示每分钟"`
...
var createScheduledAgentTaskToolDef = toolDefinition[createScheduledAgentTaskArgs](
	...
	"创建【定时会话】任务：到点后启动一个 Agent 工作台会话，并把 message 当作用户消息直接发给该会话。核心参数是 title + message；其它参数只是目录、时间、附件、模型等配置。每 N 秒执行请传 interval_seconds，例如每 5 分钟传 interval_seconds=300；不要把这些参数包进 body。适合“模型库每 6 小时巡检全球厂商和新模型并可信写入”“每天整理新闻日报”“每周读取业务数据生成周报”“定期巡检工单/订单/库存异常”这类需要 Agent 判断、查询、总结、维护长期数据或组合多个动作的任务。定时会话可以编排当前目录、本空间其他目录、其他空间函数、系统工具和连接器函数。定时会话是无人值守执行，运行时用户无法回答问题；创建前必须把范围、可用资源/函数、预期使用工具清单、必要参数、按业务场景裁剪的质量控制策略、输出格式、失败处理和风险确认问清楚，并全部写进 message，不能留下“到时候问用户/等待确认”。message 必须明确列出预计使用哪些工具以及何时使用，例如 change_role、read_dir/search、web_search、run_table_search、run_table_create/run_table_update、run_form_submit、run_chart_query、send_notification；定时会话运行时会注入任务创建人/请求用户作为默认通知对象；send_notification 在定时会话中必须显式传 to_users，通知创建人、当前用户或“我”时写明使用创建人 username，首次基准记录、无变化结果、普通状态报告默认不通知；质量规则要结合业务，不要把示例机械套到所有任务。不要用于已明确的单个 Form/Table/Chart 函数调用；如果目标能写成固定函数路径 + 固定 body，请用 create_scheduled_function_task，更稳定也更便宜。创建前必须确认 full_code_path 是目标工作空间/目录，不是具体函数路径；message 不能包含未授权的跨应用操作。",
)
