...
type scheduledTaskScheduleArgs struct {
	...
	Timezone        string `json:"timezone" schema_desc:"时区，默认 Asia/Shanghai；cron 和无时区 run_at 会使用它"`
...
type createScheduledAgentTaskArgs struct {
	...
	Message            string `json:"message" schema_desc:"到点后直接发送给工作台会话的完整用户消息。把要 Agent 做的事、范围、输出格式、注意事项都写在这一段里；运行时用户不在线，不能写“到时候问我/等待确认”。如果只是固定调用一个函数，请改用 create_scheduled_function_task"`
	ScheduleType       string `json:"schedule_type" schema_desc:"计划类型，可不传。只填 run_at 会自动当一次性任务；只填 cron_expr 会自动当 cron 周期任务；只填 interval_seconds 会自动当每隔 N 秒任务" schema_enum:"atime,cron,every"`
	RunAt              string `json:"run_at" schema_desc:"一次性任务的执行时间，例如 2026-06-10 09:30:00 或 RFC3339；无时区时按 timezone 解析"`
	CronExpr           string `json:"cron_expr" schema_desc:"cron 周期表达式，例如每分钟 * * * * *，每天 9 点 0 9 * * *"`
	IntervalSeconds    int64  `json:"interval_seconds" schema_desc:"每隔多少秒执行一次，例如 60 表示每分钟"`
...
var createScheduledAgentTaskToolDef = toolDefinition[createScheduledAgentTaskArgs](
	...
	"创建【定时会话】任务：到点后启动一个 Agent 工作台会话，并把 message 当作用户消息直接发给该会话。核心参数是 title + message；其它参数只是目录、时间、附件、模型等配置。每 N 秒执行请传 interval_seconds，例如每 5 分钟传 interval_seconds=300；不要把这些参数包进 body。适合“每天巡检这个应用并总结异常”“每周分析数据并给结论”“定期检查工单状态并写报告”这类需要 Agent 判断、查询、总结或组合多个动作的任务。定时会话是无人值守执行，运行时用户无法回答问题；创建前必须把范围、工具路径、必要参数、输出格式、失败处理和风险确认问清楚，并全部写进 message，不能留下“到时候问用户/等待确认”。不要用于已明确的单个 Form/Table/Chart 函数调用；如果目标能写成固定函数路径 + 固定 body，请用 create_scheduled_function_task，更稳定也更便宜。创建前必须确认 full_code_path 是目标工作空间/目录，不是具体函数路径；message 不能包含未授权的跨应用操作。",
)
...
func rejectCreateScheduledAgentTaskUnknownArgs(args map[string]interface{}) error {
	...
	for key := range args {
		if _, ok := allowed[key]; !ok {
			return fmt.Errorf("不支持参数 %q；定时会话只接收顶层 title、message、full_code_path 和计划配置，间隔执行请用 interval_seconds", key)
		}
	}
	return nil
}
...
func parseScheduledRunAt(raw string, timezone string) (time.Time, error) {
	...
	location, err := time.LoadLocation(scheduledTaskTimezone(timezone))
	if err != nil {
		location = time.Local
	}
...
func scheduledTaskTimezone(raw string) string {
	if tz := strings.TrimSpace(raw); tz != "" {
		return tz
	}
