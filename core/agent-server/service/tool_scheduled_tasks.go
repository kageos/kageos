...
type createScheduledAgentTaskArgs struct {
	FullCodePath       string `json:"full_code_path" schema_desc:"数字员工（Agent 任务）值守的工作台目录或应用路径，例如 /system/app；通常就是当前 execute_directory，不是具体 .form/.table/.chart 函数路径"`
	Title              string `json:"title" schema_desc:"数字员工在界面上的岗位或名称，用来在列表里识别这条 Agent 任务，例如 每日热点巡检员"`
	Message            string `json:"message" schema_desc:"数字员工到点后遵循的完整工作说明。必须像无人值守运行手册一样写清：岗位性质、长期目标、绑定服务目录/资源（相对路径优先：当前服务目录用 <./xxx.form>，同一服务目录的兄弟服务目录用 <../other/xxx.table>；用户明确要求跨工作空间时可用 </完整路径>，但要注明不可移植、复制后需重新绑定；内置工具用 <tool:send_notification>）、可用的其他服务目录/函数/连接器/内置工具、预期使用工具清单（如 change_role/search/read_dir/web_search/run_table_search/run_table_create/run_table_update/run_form_submit/run_chart_query/send_notification）、工作步骤、按业务场景裁剪的质量控制策略、失败处理、输出格式、通知规则。底层 Agent 会话会注入任务创建人/请求用户作为默认通知对象；需要通知创建人、当前用户或“我”时，send_notification 可省略 to_users，也可显式传创建人 username；首次基准记录、无变化结果、普通状态报告默认不通知。运行时用户不在线，不能写“到时候问我/等待确认”。如果只是固定调用一个函数，请改用 create_scheduled_function_task"`
	ScheduleType       string `json:"schedule_type" schema_desc:"计划类型，可不传。只填 run_at 会自动当一次性任务；只填 cron_expr 会自动当 cron 周期任务；只填 interval_seconds 会自动当每隔 N 秒任务" schema_enum:"atime,cron,every"`
	RunAt              string `json:"run_at" schema_desc:"一次性任务的执行时间，例如 2026-06-10 09:30:00 或 RFC3339；无时区时按 timezone 解析"`
	CronExpr           string `json:"cron_expr" schema_desc:"cron 周期表达式，例如每分钟 * * * * *，每天 9 点 0 9 * * *"`
	IntervalSeconds    int64  `json:"interval_seconds" schema_desc:"每隔多少秒执行一次，例如 60 表示每分钟"`
	...
	Description        string `json:"description" schema_desc:"数字员工职责摘要"`
	IdempotencyKey     string `json:"idempotency_key" schema_desc:"可选幂等 key；不传则根据目标、计划和参数生成"`
...
type updateScheduledAgentTaskArgs struct {
	...
	Title              string `json:"title" schema_desc:"可选：更新员工名称或岗位，不传则不更新"`
	Message            string `json:"message" schema_desc:"可选：更新数字员工的完整工作说明。请提供完整的新内容"`
	...
	Description        string `json:"description" schema_desc:"可选：数字员工职责摘要"`
}
...
var createScheduledAgentTaskToolDef = toolDefinition[createScheduledAgentTaskArgs](
	...
	"创建【数字员工（Agent 任务）】：产品界面和用户语言里的“数字员工”“值守员工”就是 agent.session Agent 任务；用户说“给这个服务目录创建一个数字员工”时使用本工具，不要创建用户、角色、服务目录或普通函数任务。到点后会启动一个 Agent 工作台会话，并把 message 当作执行说明交给工作台 Agent；这也是数字员工的完整职责说明。核心参数是 title + message；title 是员工岗位/名称，其它参数只是服务目录、时间、附件、模型等配置。每 N 秒执行请传 interval_seconds，例如每 5 分钟传 interval_seconds=300；不要把这些参数包进 body。适合“模型库每 6 小时巡检全球厂商和新模型并可信写入”“每天整理新闻日报”“每周读取业务数据生成周报”“定期巡检工单/订单/库存异常”这类需要 Agent 判断、查询、总结、维护长期数据或组合多个动作的任务。Agent 任务可以编排当前服务目录、同一服务目录中的其他服务目录、用户明确授权的其他工作空间、连接器函数和内置 Agent 工具；message 中资源引用遵循相对路径优先，当前服务目录写成 <./xxx.form>，同一服务目录的兄弟服务目录写成 <../other/xxx.table>。只有用户明确要求跨工作空间绑定时才使用 </完整路径>，并注明该依赖不可移植、复制后需重新绑定；内置工具用 <tool:send_notification> 标记。Agent 任务是无人值守执行，运行时用户无法回答问题；创建前必须把范围、可用资源/函数、预期使用工具清单、必要参数、按业务场景裁剪的质量控制策略、输出格式、失败处理和风险确认问清楚，并全部写进 message，不能留下“到时候问用户/等待确认”。message 必须明确列出预计使用哪些工具以及何时使用，例如 change_role、read_dir/search、web_search、run_table_search、run_table_create/run_table_update、run_form_submit、run_chart_query、send_notification；写给人读的说明里可把通知工具标成 <tool:send_notification>，真实工具名仍是 send_notification。Agent 任务运行时会注入任务创建人/请求用户作为默认通知对象；send_notification 通知创建人、当前用户或“我”时可省略 to_users，也可显式传创建人 username；首次基准记录、无变化结果、普通状态报告默认不通知；质量规则要结合业务，不要把示例机械套到所有任务。不要用于已明确的单个 Form/Table/Chart 函数调用；如果目标能写成固定函数路径 + 固定 body，应说明函数任务更稳定也更便宜，并在用户同意后使用 create_scheduled_function_task，不能静默替换用户明确要求的数字员工。创建前必须确认 full_code_path 是目标工作空间/服务目录，不是具体函数路径；message 不能包含未授权的跨应用操作。",
)

var updateScheduledAgentTaskToolDef = toolDefinition[updateScheduledAgentTaskArgs](
	"update_scheduled_agent_task",
	"更新【数字员工】配置：修改员工名称、工作说明（message）或执行计划。如果不修改计划，请不要传 schedule_type 及相关的 cron_expr 等参数。更新工作说明时，必须提供完整的新内容，不能只写追加的部分。",
)
