...
type createScheduledAgentTaskArgs struct {
	FullCodePath       string `json:"full_code_path" schema_desc:"智能员工（Agent 任务）值守的工作台目录或应用路径，例如 /system/app；通常就是当前 execute_directory，不是具体 .form/.table/.chart 函数路径"`
	Title              string `json:"title" schema_desc:"智能员工在界面上的岗位或名称，用来在列表里识别这条 Agent 任务，例如 每日热点巡检员"`
