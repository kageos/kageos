...
- `before_enter`：进入角色前补齐角色所需上下文，例如应用执行获取当前应用函数 schema。
...
- `app_operator.before_enter_capabilities`：在进入 `app_operator` 前按 `execute_directory` 搜索当前应用下的 Table/Form/Chart，生成 `app_capabilities` 能力快照和 `executed_hooks`，并把函数路径、运行工具、字段摘要写入标准交接的关键信息。失败时不阻断切换，但会要求下一步显式调用 `search(full_code_path=change_role.execute_directory, resource_type=function)`。
