...
- 非 CRUD 逻辑必须先确认并写清楚，包括状态计算、唯一性/重复提交、权限/只读、跨表写入、事务副作用、统计口径、校验和异常边界；不明确就先追问，不输出含糊 PRD。
...
输出时按这个结构替换业务内容，不要新增顶层字段。注意：表格示例字段必须来自 ` + "`fields`" + `；大多数表格搜索条件都包含按记录创建时间查询的 ` + "`创建开始时间`" + `、` + "`创建结束时间`" + `，以及按用户查询的 ` + "`创建人`" + ` 或业务用户字段。
