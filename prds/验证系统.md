...
我们函数详情的每个字段都有validation标签，这个标签来标识这个字段的验证规则是什么
...
validation的标签值是完全用的go的
github.com/go-playground/validator/v10 这个库来实现的，后端会用这个库对输入的字段进行验证（后端兜底）
前端需要保证和后端的验证逻辑是一致的，
...
例如后端的这两个字段，解析后函数详情里字段
card_number的validation是required,min=6,max=20，（必填，最小6个字符，最大20字符）
customer_name的validation是required,min=2,max=20 （必填，最小2个字符，最大20字符）

下面是github.com/go-playground/validator/v10的所有标签，和文档
预期我们可以先实现主要的即可，太复杂的可以先放一下无所谓
...
这个里面有个比较特殊的case就条件渲染，这个关系到字段是否需要展示
### 条件验证

| 规则 | 说明 | 示例 | 字段显示                      |
|------|------|------|---------------------------|
| `required_if=Field value` | 当指定字段等于指定值时必填 | `validate:"required_if=IsVip true"` | IsVip=true时显示,否则不显示       |
| `required_unless=Field value` | 除非指定字段等于指定值，否则必填 | `validate:"required_unless=UserType guest"` | UserType=guest 时候不显示，否则显示 |
| `required_with=Field` | 当指定字段有值时必填 | `validate:"required_with=Phone"` | 当Phone有值时候显示              |
| `required_without=Field` | 当指定字段无值时必填 | `validate:"required_without=Email"` | 当Email无值时候显示              |

但是这里有个比较坑的点就是，后端写的是IsVip这种对应的是go的字段名称，但是我们后端返回的是code是json的标签，所以我们在做映射时候还需一个转换？这个你想一下该怎么办比较好啊？先分析一下？