...
用户：我需要一个工单管理系统
1分钟后，大模型（[llm-coder](../core/llm-coder) 还没实现）基于我的sdk生成了下面的crm_ticket.go文件，这是一个table类型的模版系统
...
最终这个go的table模版文件 模版解析出的json是这样的
...
这样的json其实是可以直接渲染成界面的，



## table类型的模版

table模版也是包含请求参数和响应参数

请求参数一般是null
假如非空的话，都是一些查询字段其渲染的效果跟下面响应参数并无差别，响应字段里的字段其实是对应后端的一张表，所以表内字段可以在search里打标签
响应参数是这样的
```json
[
  {
