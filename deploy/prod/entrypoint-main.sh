...
if [[ -z "${KAGEOS_CONTROL_PLANE_SECRET:-}" ]]; then
  echo "WARN: KAGEOS_CONTROL_PLANE_SECRET 未设置，暂时回退到 JWT_SECRET；请尽快生成独立密钥完成轮换" >&2
  KAGEOS_CONTROL_PLANE_SECRET="$JWT_SECRET"
fi
export KAGEOS_CONTROL_PLANE_SECRET
