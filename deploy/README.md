...
| 方式 | 目录 | 说明 |
|------|------|------|
| **客户主站一键（Compose）** | [customer/](customer/) | **不含 Hub**：宿主机 **`cp .env.example .env`** 填必填项 → **`./check-env.sh`** → **`podman compose`** / **`docker compose`** 起 MySQL/NATS/MinIO + **main** 胖镜像（**无 Python 依赖**）；说明见 [customer/README.md](customer/README.md)。 |
