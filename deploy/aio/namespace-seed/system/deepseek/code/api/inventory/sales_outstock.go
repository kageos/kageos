...
// SalesOutStockReq 销售出库请求
type SalesOutStockReq struct {
	OrderNo string `json:"order_no" widget:"name:销售单号;type:select;placeholder:请选择待出库的销售单" validate:"required" callback:"OnSelectFuzzy"`
}

// SalesOutStockResp 销售出库响应
type SalesOutStockResp struct {
	Result    string `json:"result" widget:"name:出库结果;type:input"`
	ItemCount int    `json:"item_count" widget:"name:出库商品数;type:integer"`
}

var SalesOutStockTemplate = &app.FormTemplate{
	BaseConfig: app.BaseConfig{
		Name:     "销售出库",
		Request:  &SalesOutStockReq{},
		Response: &SalesOutStockResp{},
		OnSelectFuzzyMap: map[string]app.OnSelectFuzzy{
			"order_no": onSelectFuzzyPendingSalesOrder,
		},
	},
}

func onSelectFuzzyPendingSalesOrder(ctx *app.Context, req *callback.OnSelectFuzzyReq) (*callback.OnSelectFuzzyResp, error) {
	db := ctx.GetGormDB()
	var orders []SalesOrder
	queryDB := db.Model(&SalesOrder{}).Where("status = ?", "待出库")
	if req.IsByValue() {
		...
		keyword := req.Keyword()
		if keyword != "" {
			queryDB = queryDB.Where("order_no LIKE ? OR customer LIKE ?", "%"+keyword+"%", "%"+keyword+"%")
		}
	...
	for _, o := range orders {
		items = append(items, &callback.SelectFuzzyItem{
			...
			DisplayInfo: map[string]interface{}{
				"客户":   o.Customer,
				"销售金额": o.TotalAmount,
				"销售日期": o.OrderDate.Time().Format("2006-01-02 15:04"),
			},
		})
	}
...
// SalesOutStock 销售出库处理
func SalesOutStock(ctx *app.Context, resp response.Response) error {
	db := ctx.GetGormDB()
	var req SalesOutStockReq
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		var so SalesOrder
		if err := tx.Where("order_no = ?", req.OrderNo).First(&so).Error; err != nil {
			result = fmt.Sprintf("销售单 %s 不存在", req.OrderNo)
			return fmt.Errorf("销售单 %s 不存在", req.OrderNo)
		}
		if so.Status != "待出库" {
			result = fmt.Sprintf("销售单 %s 当前状态为%s，非待出库状态", req.OrderNo, so.Status)
			return fmt.Errorf("销售单状态异常: %s", so.Status)
		}

		lines := strings.Split(strings.TrimSpace(so.ItemsDetail), "\n")
		...
		for _, item := range items {
			...
			if product.Stock < item.quantity {
				result = fmt.Sprintf("商品 %s 库存不足（当前库存：%d，需要：%d）", item.name, product.Stock, item.quantity)
				return fmt.Errorf("库存不足: %s", item.name)
			}
			// 扣减库存
		...
		if err := tx.Model(&so).Updates(map[string]interface{}{
			...
			result = "更新销售单状态失败"
			return fmt.Errorf("更新销售单状态失败: %w", err)
		}
	...
	if err != nil {
		return resp.Form(&SalesOutStockResp{
			Result:    result,
			ItemCount: itemCount,
		}).Build()
	}
...
func init() {
	packageContext.POST("sales_outstock.form", SalesOutStock, SalesOutStockTemplate)
}
