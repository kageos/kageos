...
type CertCFRequest struct {
	...
	CloudflareZoneID    string     `json:"cloudflare_zone_id" gorm:"column:cloudflare_zone_id;type:varchar(80);comment:Cloudflare Zone ID" widget:"name:Zone ID;type:text" hide:"create,update"`
	CloudflareZoneName  string     `json:"cloudflare_zone_name" gorm:"column:cloudflare_zone_name;type:varchar(255);comment:Cloudflare Zone" widget:"name:Zone;type:text" hide:"create,update"`
	CloudflareRecordIDs string     `json:"cloudflare_record_ids" gorm:"column:cloudflare_record_ids;type:text;comment:DNS记录ID" widget:"name:DNS记录ID;type:text_area" hide:"create,update"`
...
func (CertCFRequest) TableName() string {
	return "cf_cert_request"
}

type CertCFRequestListReq struct {
...
func CertCFRequestList(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	var req CertCFRequestListReq
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	queryDB := db.Model(&CertCFRequest{})
...
func CertCFIssue(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	...
	var domain CertCFDomain
	if err := db.First(&domain, req.DomainID).Error; err != nil {
		return fmt.Errorf("证书域名不存在")
	}
	logger.Infof(ctx, "[CertManager][Cloudflare] manual issue submitted domain_id=%d domain=%s request_type=%s wait=%d poll=%d cleanup=%v user=%s",
	...
	if err := ensureNoRunningRequest(db, domain.ID); err != nil {
		logger.Warnf(ctx, "[CertManager][Cloudflare] manual issue blocked by running request domain_id=%d domain=%s err=%v", domain.ID, domain.Domain, err)
		return err
	}
	...
	logger.Infof(ctx, "[CertManager][Cloudflare] request record created request_id=%d domain_id=%d domain=%s type=%s",
	...
	if issueErr != nil {
		logger.Errorf(ctx, "[CertManager][Cloudflare] manual issue failed request_id=%d domain=%s err=%v", reqRecord.ID, domain.Domain, issueErr)
	...
	logger.Infof(ctx, "[CertManager][Cloudflare] manual issue succeeded request_id=%d asset_id=%d domain=%s", reqRecord.ID, asset.ID, domain.Domain)
	link, _ := ctx.BuildFunctionUrlWithText("assets.table", CertCFAsset{ID: asset.ID}, "查看证书资产")
	return resp.Form(&CertCFIssueResp{
...
type CertCFSweepReq struct {
	WaitSeconds         int  `json:"wait_seconds" widget:"name:DNS等待秒数;type:integer;min:30;max:900;render_default:300"`
	PollIntervalSeconds int  `json:"poll_interval_seconds" widget:"name:DNS轮询间隔;type:integer;min:5;max:120;render_default:15"`
	CleanupChallenge    bool `json:"cleanup_challenge" widget:"name:签发后删除TXT;type:switch;render_default:true"`
}

type CertCFSweepResp struct {
...
func CertCFSweep(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	req := CertCFSweepReq{WaitSeconds: defaultDNSWaitSeconds, PollIntervalSeconds: defaultDNSPollSeconds, CleanupChallenge: true}
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	var domains []CertCFDomain
	if err := db.Where("enabled = ? AND auto_renew = ?", true, true).Order("domain ASC").Find(&domains).Error; err != nil {
		return err
	}
	result := CertCFSweepResp{CheckedCount: len(domains)}
	logger.Infof(ctx, "[CertManager][Cloudflare] auto renew sweep start checked=%d wait=%d poll=%d cleanup=%v",
		len(domains), req.WaitSeconds, req.PollIntervalSeconds, req.CleanupChallenge)
	for i := range domains {
		domain := domains[i]
		if !domainNeedsRenewal(domain) {
			logger.Debugf(ctx, "[CertManager][Cloudflare] auto renew skipped no renewal needed domain_id=%d domain=%s days_left=%d renew_before=%d",
				domain.ID, domain.Domain, domain.CurrentDaysLeft, domain.RenewBeforeDays)
			result.SkippedCount++
			continue
		}
		if err := ensureNoRunningRequest(db, domain.ID); err != nil {
			logger.Warnf(ctx, "[CertManager][Cloudflare] auto renew skipped running request domain_id=%d domain=%s err=%v", domain.ID, domain.Domain, err)
			result.SkippedCount++
			continue
		}
		...
		logger.Infof(ctx, "[CertManager][Cloudflare] auto renew request created request_id=%d domain_id=%d domain=%s",
		...
		if _, err := runIssueRequest(ctx, db, &domain, reqRecord, certificateIssueOptions{
			...
			logger.Errorf(ctx, "[CertManager][Cloudflare] auto renew failed request_id=%d domain=%s err=%v", reqRecord.ID, domain.Domain, err)
		...
		logger.Infof(ctx, "[CertManager][Cloudflare] auto renew succeeded request_id=%d domain=%s", reqRecord.ID, domain.Domain)
		result.IssuedCount++
	}
	logger.Infof(ctx, "[CertManager][Cloudflare] auto renew sweep finished checked=%d skipped=%d issued=%d failed=%d",
		result.CheckedCount, result.SkippedCount, result.IssuedCount, result.FailedCount)
	return resp.Form(&result).Build()
}

func runIssueRequest(ctx *app.Context, db *gorm.DB, domain *CertCFDomain, reqRecord *CertCFRequest, opts certificateIssueOptions) (*CertCFAsset, error) {
	now := time.Now()
	logger.Infof(ctx, "[CertManager][Cloudflare] run issue request start request_id=%d domain_id=%d domain=%s type=%s",
		reqRecord.ID, domain.ID, domain.Domain, reqRecord.RequestType)
	_ = db.Model(&CertCFRequest{}).Where("id = ?", reqRecord.ID).Updates(map[string]interface{}{
		"status":     requestStatusRunning,
		"started_at": types.Time(now),
	}).Error
	issued, err := issueCertificateForRequest(ctx, db, domain, reqRecord, opts)
	if err != nil {
		logger.Errorf(ctx, "[CertManager][Cloudflare] issue certificate failed request_id=%d domain=%s err=%v", reqRecord.ID, domain.Domain, err)
		return nil, err
	}
	asset, err := createAssetFromIssuedCertificate(ctx, db, domain, reqRecord, issued)
	if err != nil {
		logger.Errorf(ctx, "[CertManager][Cloudflare] create asset failed request_id=%d domain=%s err=%v", reqRecord.ID, domain.Domain, err)
		return nil, err
	}
	_ = db.Model(&CertCFRequest{}).Where("id = ?", reqRecord.ID).Updates(map[string]interface{}{
		"status":       requestStatusIssued,
	...
	logger.Infof(ctx, "[CertManager][Cloudflare] run issue request finished request_id=%d asset_id=%d domain=%s elapsed=%s",
		reqRecord.ID, asset.ID, domain.Domain, time.Since(now).Round(time.Millisecond))
	return asset, nil
}

func createRequestRecord(ctx *app.Context, db *gorm.DB, domain *CertCFDomain, requestType string, remark string) (*CertCFRequest, error) {
	if requestType == "" {
...
func updateRequestStatus(db *gorm.DB, requestID int, status string, message string, extra map[string]interface{}) {
	...
	_ = db.Model(&CertCFRequest{}).Where("id = ?", requestID).Updates(updates).Error
}

func appendChallengeRecord(db *gorm.DB, requestID int, zone *cloudflareZone, record *cloudflareDNSRecord, name string, value string) {
	var row CertCFRequest
	if err := db.First(&row, requestID).Error; err != nil {
		return
	}
	recordRef := zone.ID + ":" + record.ID
	records := splitList(row.CloudflareRecordIDs)
	...
	updates := map[string]interface{}{
		...
		"cloudflare_zone_id":    firstNonEmpty(row.CloudflareZoneID, zone.ID),
		"cloudflare_zone_name":  firstNonEmpty(row.CloudflareZoneName, zone.Name),
		"cloudflare_record_ids": strings.Join(records, ","),
	}
	_ = db.Model(&CertCFRequest{}).Where("id = ?", requestID).Updates(updates).Error
}

func markRequestFailed(db *gorm.DB, requestID int, err error) {
	_ = db.Model(&CertCFRequest{}).Where("id = ?", requestID).Updates(map[string]interface{}{
...
func domainNeedsRenewal(domain CertCFDomain) bool {
...
func sendIssueSuccessMessage(ctx *app.Context, domain *CertCFDomain, reqRecord *CertCFRequest, asset *CertCFAsset) {
	...
	_ = ctx.SendMessage(&app.SendMessageOpts{
		ToUsers: toUsers,
		Title:   "Cloudflare 证书签发成功",
		Content: content,
	})
}

func sendIssueFailureMessage(ctx *app.Context, domain *CertCFDomain, reqRecord *CertCFRequest, err error) {
	toUsers := joinCertificateNotifyUsers(*domain)
	if toUsers == "" {
		return
	}
	...
	_ = ctx.SendMessage(&app.SendMessageOpts{
		ToUsers: toUsers,
		Title:   "Cloudflare 证书签发失败",
		Content: content,
	})
}

func certCFRequestOnSelectFuzzy(ctx *app.Context, req *callback.OnSelectFuzzyReq) (*callback.OnSelectFuzzyResp, error) {
	db, err := certManagerDB(ctx)
	if err != nil {
		return nil, err
	}
...
var CertCFRequestListTemplate = &app.TableTemplate{
	BaseConfig: app.BaseConfig{
		Name:         "证书申请与续期记录",
		Desc:         "记录 Cloudflare DNS-01 自动签发和自动续期的状态、DNS challenge、错误信息和资产关联。",
		Tags:         []string{"Cloudflare", "证书申请", "自动续期"},
		Request:      &CertCFRequestListReq{},
		Response:     query.PaginatedTable[[]CertCFRequest]{},
		CreateTables: certManagerTables(),
	},
}

func init() {
	packageContext.GET("requests.table", CertCFRequestList, CertCFRequestListTemplate)
	packageContext.POST("issue.form", CertCFIssue, &app.FormTemplate{
		BaseConfig: app.BaseConfig{
			Name:         "申请/续期证书",
			Desc:         "为域名自动执行 Let's Encrypt DNS-01 签发：写入 Cloudflare TXT、等待 DNS 生效、提交 ACME 验证、签发证书并归档文件。不会自动部署证书。",
			Tags:         []string{"Cloudflare", "自动签发", "DNS-01"},
			Request:      &CertCFIssueReq{},
			Response:     &CertCFIssueResp{},
			CreateTables: certManagerTables(),
			OnSelectFuzzyMap: map[string]app.OnSelectFuzzy{
				"domain_id": certCFDomainOnSelectFuzzy,
			},
		},
	})
	packageContext.POST("auto_renew_sweep.form", CertCFSweep, &app.FormTemplate{
		BaseConfig: app.BaseConfig{
			Name:         "证书自动续期巡检",
			Desc:         "扫描启用自动续期的域名，发现未签发或即将过期时自动走 Cloudflare DNS-01 签发流程并归档证书文件。",
			Tags:         []string{"Cloudflare", "自动续期", "定时任务"},
			Request:      &CertCFSweepReq{},
			Response:     &CertCFSweepResp{},
			CreateTables: certManagerTables(),
		},
		Schedules: []app.FormSchedule{
			{
				Code:        "cloudflare_cert_auto_renew_daily",
				Title:       "Cloudflare 证书每日自动续期巡检",
				Description: "每天 03:00 检查启用自动续期的域名，快过期时自动签发新证书并归档。",
				CronExpr:    "0 3 * * *",
				Body:        CertCFSweepReq{WaitSeconds: defaultDNSWaitSeconds, PollIntervalSeconds: defaultDNSPollSeconds, CleanupChallenge: true},
			},
		},
	})
}
