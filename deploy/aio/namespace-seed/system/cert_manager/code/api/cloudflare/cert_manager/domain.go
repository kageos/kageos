...
type CertCFDomain struct {
	...
	ConfigID        int           `json:"config_id" gorm:"column:config_id;index;comment:Cloudflare配置ID" widget:"name:Cloudflare配置;type:select" validate:"required" callback:"OnSelectFuzzy"`
	Config          *CertCFConfig `json:"-" widget:"-" gorm:"foreignKey:ConfigID;references:ID"`
	ConfigName      string        `json:"config_name" gorm:"column:config_name;type:varchar(120);comment:Cloudflare配置名称" widget:"name:Cloudflare配置;type:text" hide:"create,update"`
...
func (CertCFDomain) TableName() string {
	return "cf_cert_domain"
}

type CertCFDomainListReq struct {
	Domain            string `json:"domain" form:"domain" widget:"name:域名;type:input"`
	ConfigName        string `json:"config_name" form:"config_name" widget:"name:Cloudflare配置;type:input"`
...
func CertCFDomainList(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	var req CertCFDomainListReq
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	queryDB := db.Model(&CertCFDomain{})
...
var CertCFDomainListTemplate = &app.TableTemplate{
	BaseConfig: app.BaseConfig{
		Name:         "证书域名管理",
		Desc:         "维护由 Cloudflare DNS 自动验证和续期的证书域名。系统会自动写入 DNS-01 TXT、签发证书并归档文件，但不会自动部署。",
		Tags:         []string{"Cloudflare", "证书管理", "域名"},
		Request:      &CertCFDomainListReq{},
		Response:     query.PaginatedTable[[]CertCFDomain]{},
		CreateTables: certManagerTables(),
		OnSelectFuzzyMap: map[string]app.OnSelectFuzzy{
			"config_id": certCFConfigOnSelectFuzzy,
		},
	},
	AutoCrudTable: &CertCFDomain{},
	OnTableAddRow: func(ctx *app.Context, req *callback.OnTableAddRowReq) (*callback.OnTableAddRowResp, error) {
		...
		if err := db.Create(&row).Error; err != nil {
			logger.Errorf(ctx, "Create cert cloudflare domain err: %v", err)
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Cloudflare] domain created domain_id=%d domain=%s config_id=%d owner=%s auto_renew=%v enabled=%v",
	...
	OnTableUpdateRow: func(ctx *app.Context, req *callback.OnTableUpdateRowReq) (*callback.OnTableUpdateRowResp, error) {
		...
		if req.IsFieldUpdated("config_id") {
			...
			if err := db.First(&cfg, updateFields.ConfigID).Error; err != nil {
				return nil, fmt.Errorf("Cloudflare 配置不存在")
			}
		...
		if err := db.Model(&CertCFDomain{}).Where("id = ?", req.GetId()).Updates(updates).Error; err != nil {
			logger.Errorf(ctx, "Update cert cloudflare domain err: %v", err)
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Cloudflare] domain updated domain_id=%d changed_fields=%d", req.GetId(), len(updates))
	...
	OnTableDeleteRows: func(ctx *app.Context, req *callback.OnTableDeleteRowsReq) (*callback.OnTableDeleteRowsResp, error) {
		...
		if err := db.Model(&CertCFAsset{}).Where("domain_id in ?", req.GetIds()).Count(&assetCount).Error; err != nil {
			return nil, fmt.Errorf("检查证书资产失败: %w", err)
		}
		if assetCount > 0 {
			return nil, fmt.Errorf("域名下存在证书资产，请先确认是否需要保留资产记录")
		}
		logger.Infof(ctx, "[CertManager][Cloudflare] domain delete requested ids=%v", req.GetIds())
		if err := db.Delete(&CertCFDomain{}, "id in ?", req.GetIds()).Error; err != nil {
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Cloudflare] domain deleted ids=%v", req.GetIds())
		return &callback.OnTableDeleteRowsResp{}, nil
	},
}

func normalizeDomainRow(ctx *app.Context, db *gorm.DB, row *CertCFDomain) error {
	...
	row.Domain = domain
	if row.ConfigID <= 0 {
		return fmt.Errorf("请选择 Cloudflare 配置")
	}
	var cfg CertCFConfig
	if err := db.First(&cfg, row.ConfigID).Error; err != nil {
		return fmt.Errorf("Cloudflare 配置不存在")
	}
...
func certCFDomainOnSelectFuzzy(ctx *app.Context, req *callback.OnSelectFuzzyReq) (*callback.OnSelectFuzzyResp, error) {
	db, err := certManagerDB(ctx)
	if err != nil {
		return nil, err
	}
	...
	return &callback.OnSelectFuzzyResp{
		...
		Statistics: map[string]interface{}{
			...
			"Cloudflare配置": statistics.Value("Cloudflare配置"),
...
func init() {
	packageContext.GET("domains.table", CertCFDomainList, CertCFDomainListTemplate)
}
