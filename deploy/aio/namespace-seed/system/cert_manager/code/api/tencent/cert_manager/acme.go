...
func issueCertificateForRequest(ctx *app.Context, db *gorm.DB, domain *CertTencentDomain, reqRecord *CertTencentRequest, opts certificateIssueOptions) (*issuedCertificate, error) {
	...
	startedAt := time.Now()
	logger.Infof(ctx, "[CertManager][Tencent] issue start request_id=%d domain_id=%d domain=%s config_id=%d wait=%ds poll=%ds cleanup=%v",
		reqRecord.ID, domain.ID, domain.Domain, domain.ConfigID, opts.WaitSeconds, opts.PollIntervalSeconds, opts.CleanupChallenge)
	cfg, token, err := loadCertTencentConfig(ctx, domain.ConfigID)
	if err != nil {
		return nil, err
	}
	logger.Infof(ctx, "[CertManager][Tencent] config loaded request_id=%d config_id=%d config=%s env=%s",
	...
	logger.Infof(ctx, "[CertManager][Tencent] creating ACME order request_id=%d names=%s", reqRecord.ID, strings.Join(names, ","))
	...
	logger.Infof(ctx, "[CertManager][Tencent] ACME order created request_id=%d status=%s authz_count=%d",
	...
	logger.Infof(ctx, "[CertManager][Tencent] DNS challenges prepared request_id=%d count=%d", reqRecord.ID, len(challenges))
	if opts.CleanupChallenge {
		defer cleanupDNSChallenges(ctx, token, challenges)
	}
	if len(challenges) > 0 {
		updateRequestStatus(db, reqRecord.ID, requestStatusWaitDNS, "等待腾讯云 DNSPod TXT 记录生效", map[string]interface{}{
			"challenge_name":  challenges[0].Name,
			"challenge_value": challenges[0].Value,
		})
		logger.Infof(ctx, "[CertManager][Tencent] waiting DNS propagation request_id=%d count=%d timeout=%ds interval=%ds first_name=%s",
		...
		logger.Infof(ctx, "[CertManager][Tencent] DNS propagation visible request_id=%d count=%d", reqRecord.ID, len(challenges))
		...
		logger.Infof(ctx, "[CertManager][Tencent] ACME challenges accepted request_id=%d count=%d", reqRecord.ID, len(challenges))
	}
	...
	logger.Infof(ctx, "[CertManager][Tencent] ACME order ready request_id=%d status=%s", reqRecord.ID, order.Status)
	...
	logger.Infof(ctx, "[CertManager][Tencent] finalizing certificate request_id=%d names=%s", reqRecord.ID, strings.Join(names, ","))
	...
	logger.Infof(ctx, "[CertManager][Tencent] issue finished request_id=%d domain=%s cert_url_present=%v elapsed=%s",
...
func prepareDNSChallenges(ctx *app.Context, db *gorm.DB, client *acme.Client, token string, requestID int, order *acme.Order) ([]*acmeDNSChallenge, error) {
	...
	for _, authzURL := range order.AuthzURLs {
		...
		logger.Infof(ctx, "[CertManager][Tencent] authorization loaded request_id=%d identifier=%s status=%s",
			requestID, authz.Identifier.Value, authz.Status)
		if authz.Status == acme.StatusValid {
			logger.Infof(ctx, "[CertManager][Tencent] authorization already valid request_id=%d identifier=%s",
		...
		logger.Infof(ctx, "[CertManager][Tencent] preparing TXT record request_id=%d identifier=%s name=%s",
			requestID, authz.Identifier.Value, txtName)
		zone, err := findTencentZone(token, authz.Identifier.Value)
		if err != nil {
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Tencent] zone matched request_id=%d identifier=%s zone_id=%s zone_name=%s",
			requestID, authz.Identifier.Value, zone.ID, zone.Name)
		record, err := createTencentTXTRecord(token, zone.ID, txtName, txtValue)
		if err != nil {
			return nil, fmt.Errorf("创建腾讯云 DNSPod TXT 记录失败: %w", err)
		}
		logger.Infof(ctx, "[CertManager][Tencent] TXT record created request_id=%d zone_id=%s record_id=%s name=%s",
...
func cleanupDNSChallenges(ctx *app.Context, token string, challenges []*acmeDNSChallenge) {
	for _, challenge := range challenges {
		...
		if err := deleteTencentDNSRecord(token, challenge.Zone.ID, challenge.Record.ID); err != nil {
			logger.Warnf(ctx, "delete tencent dns record failed zone=%s record=%s err=%v", challenge.Zone.ID, challenge.Record.ID, err)
			continue
		}
		logger.Infof(ctx, "[CertManager][Tencent] TXT record cleaned zone_id=%s record_id=%s name=%s",
			challenge.Zone.ID, challenge.Record.ID, challenge.Name)
	}
}

func newACMEClient(cfg *CertTencentConfig) (*acme.Client, crypto.Signer, error) {
	...
	client := &acme.Client{
		...
		UserAgent:    "KageOS Tencent Cert Manager",
	}
	return client, key, nil
}

func ensureACMEAccount(ctx context.Context, client *acme.Client, cfg *CertTencentConfig) error {
...
func writeIssuedCertificateFiles(ctx *app.Context, domain string, issued *issuedCertificate) error {
	outputDir := filepath.Join(ctx.GetFS().GetTraceOutputDir(), "tencent-cert")
