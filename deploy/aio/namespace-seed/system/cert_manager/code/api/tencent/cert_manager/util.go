...
func joinCertificateNotifyUsers(domain CertTencentDomain) string {
...
func updateDomainFromCertMeta(db *gorm.DB, domain *CertTencentDomain, meta certMetadata, checkedAt time.Time) {
	_ = db.Model(&CertTencentDomain{}).Where("id = ?", domain.ID).Updates(map[string]interface{}{
...
func updateDomainFailure(db *gorm.DB, domainID int, checkedAt time.Time, message string) {
	_ = db.Model(&CertTencentDomain{}).Where("id = ?", domainID).Updates(map[string]interface{}{
