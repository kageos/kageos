...
type CertTencentDomain struct {
	...
	ConfigID        int                `json:"config_id" gorm:"column:config_id;index;comment:腾讯云配置ID" widget:"name:腾讯云配置;type:select" validate:"required" callback:"OnSelectFuzzy"`
	Config          *CertTencentConfig `json:"-" widget:"-" gorm:"foreignKey:ConfigID;references:ID"`
	ConfigName      string             `json:"config_name" gorm:"column:config_name;type:varchar(120);comment:腾讯云配置名称" widget:"name:腾讯云配置;type:text" hide:"create,update"`
...
func (CertTencentDomain) TableName() string {
	return "tencent_cert_domain"
}

type CertTencentDomainListReq struct {
	Domain            string `json:"domain" form:"domain" widget:"name:域名;type:input"`
	ConfigName        string `json:"config_name" form:"config_name" widget:"name:腾讯云配置;type:input"`
...
func CertTencentDomainList(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	var req CertTencentDomainListReq
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	queryDB := db.Model(&CertTencentDomain{})
...
var CertTencentDomainListTemplate = &app.TableTemplate{
	BaseConfig: app.BaseConfig{
		Name:         "证书域名管理",
		Desc:         "维护由腾讯云 DNSPod 自动验证和续期的证书域名。系统会自动写入 DNS-01 TXT、签发证书并归档文件，但不会自动部署。",
		Tags:         []string{"腾讯云", "DNSPod", "证书管理", "域名"},
		Request:      &CertTencentDomainListReq{},
		Response:     query.PaginatedTable[[]CertTencentDomain]{},
		CreateTables: certManagerTables(),
		OnSelectFuzzyMap: map[string]app.OnSelectFuzzy{
			"config_id": certTencentConfigOnSelectFuzzy,
		},
	},
	AutoCrudTable: &CertTencentDomain{},
	OnTableAddRow: func(ctx *app.Context, req *callback.OnTableAddRowReq) (*callback.OnTableAddRowResp, error) {
		db, err := certManagerDB(ctx)
		if err != nil {
			return nil, err
		}
		row := CertTencentDomain{
			AutoRenew:       true,
			Enabled:         true,
		...
		if err := db.Create(&row).Error; err != nil {
			logger.Errorf(ctx, "Create cert tencent domain err: %v", err)
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Tencent] domain created domain_id=%d domain=%s config_id=%d owner=%s auto_renew=%v enabled=%v",
	...
	OnTableUpdateRow: func(ctx *app.Context, req *callback.OnTableUpdateRowReq) (*callback.OnTableUpdateRowResp, error) {
		...
		var updateFields CertTencentDomain
		...
		if req.IsFieldUpdated("config_id") {
			...
			if err := db.First(&cfg, updateFields.ConfigID).Error; err != nil {
				return nil, fmt.Errorf("腾讯云配置不存在")
			}
		...
		if err := db.Model(&CertTencentDomain{}).Where("id = ?", req.GetId()).Updates(updates).Error; err != nil {
			logger.Errorf(ctx, "Update cert tencent domain err: %v", err)
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Tencent] domain updated domain_id=%d changed_fields=%d", req.GetId(), len(updates))
	...
	OnTableDeleteRows: func(ctx *app.Context, req *callback.OnTableDeleteRowsReq) (*callback.OnTableDeleteRowsResp, error) {
		...
		if err := db.Model(&CertTencentAsset{}).Where("domain_id in ?", req.GetIds()).Count(&assetCount).Error; err != nil {
			return nil, fmt.Errorf("检查证书资产失败: %w", err)
		}
		if assetCount > 0 {
			return nil, fmt.Errorf("域名下存在证书资产，请先确认是否需要保留资产记录")
		}
		logger.Infof(ctx, "[CertManager][Tencent] domain delete requested ids=%v", req.GetIds())
		if err := db.Delete(&CertTencentDomain{}, "id in ?", req.GetIds()).Error; err != nil {
			return nil, err
		}
		logger.Infof(ctx, "[CertManager][Tencent] domain deleted ids=%v", req.GetIds())
		return &callback.OnTableDeleteRowsResp{}, nil
	},
}

func normalizeDomainRow(ctx *app.Context, db *gorm.DB, row *CertTencentDomain) error {
	...
	row.Domain = domain
	if row.ConfigID <= 0 {
		return fmt.Errorf("请选择腾讯云配置")
	}
	var cfg CertTencentConfig
	if err := db.First(&cfg, row.ConfigID).Error; err != nil {
		return fmt.Errorf("腾讯云配置不存在")
	}
...
func certTencentDomainOnSelectFuzzy(ctx *app.Context, req *callback.OnSelectFuzzyReq) (*callback.OnSelectFuzzyResp, error) {
	db, err := certManagerDB(ctx)
	if err != nil {
		return nil, err
	}
...
func init() {
	packageContext.GET("domains.table", CertTencentDomainList, CertTencentDomainListTemplate)
}
