...
func (CertTencentAsset) TableName() string {
	return "tencent_cert_asset"
}

type CertTencentAssetListReq struct {
...
func CertTencentAssetList(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	var req CertTencentAssetListReq
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	queryDB := db.Model(&CertTencentAsset{})
...
var CertTencentAssetListTemplate = &app.TableTemplate{
	BaseConfig: app.BaseConfig{
		Name:         "证书资产库",
		Desc:         "保存腾讯云 DNSPod DNS-01 自动签发得到的证书文件、私钥、证书链和 ZIP 包。证书只保存和下载，不自动部署。",
		Tags:         []string{"腾讯云", "DNSPod", "证书资产", "files"},
		Request:      &CertTencentAssetListReq{},
		Response:     query.PaginatedTable[[]CertTencentAsset]{},
		CreateTables: certManagerTables(),
	},
}

func createAssetFromIssuedCertificate(ctx *app.Context, db *gorm.DB, domain *CertTencentDomain, reqRecord *CertTencentRequest, issued *issuedCertificate) (*CertTencentAsset, error) {
	...
	asset := &CertTencentAsset{
		...
		Remark:          "腾讯云 DNSPod DNS-01 自动签发入库",
	}
	if asset.ImportedBy == "" {
	...
	logger.Infof(ctx, "[CertManager][Tencent] asset created asset_id=%d request_id=%d domain_id=%d domain=%s status=%s days_left=%d not_after=%s",
		asset.ID, reqRecord.ID, domain.ID, domain.Domain, asset.Status, asset.DaysLeft, asset.NotAfter.String())
	return asset, nil
}

func init() {
	packageContext.GET("assets.table", CertTencentAssetList, CertTencentAssetListTemplate)
}
