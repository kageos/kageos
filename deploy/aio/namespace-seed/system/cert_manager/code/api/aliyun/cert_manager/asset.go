...
func (CertAliyunAsset) TableName() string {
	return "aliyun_cert_asset"
}

type CertAliyunAssetListReq struct {
...
func CertAliyunAssetList(ctx *app.Context, resp response.Response) error {
	db, err := certManagerDB(ctx)
	if err != nil {
		return err
	}
	var req CertAliyunAssetListReq
	if err := ctx.ShouldBind(&req); err != nil {
		return err
	}
	queryDB := db.Model(&CertAliyunAsset{})
...
var CertAliyunAssetListTemplate = &app.TableTemplate{
	BaseConfig: app.BaseConfig{
		Name:         "证书资产库",
		Desc:         "保存阿里云 DNS-01 自动签发得到的证书文件、私钥、证书链和 ZIP 包。证书只保存和下载，不自动部署。",
		Tags:         []string{"阿里云", "证书资产", "files"},
		Request:      &CertAliyunAssetListReq{},
		Response:     query.PaginatedTable[[]CertAliyunAsset]{},
		CreateTables: certManagerTables(),
	},
}

func createAssetFromIssuedCertificate(ctx *app.Context, db *gorm.DB, domain *CertAliyunDomain, reqRecord *CertAliyunRequest, issued *issuedCertificate) (*CertAliyunAsset, error) {
	...
	logger.Infof(ctx, "[CertManager][Aliyun] asset created asset_id=%d request_id=%d domain_id=%d domain=%s status=%s days_left=%d not_after=%s",
		asset.ID, reqRecord.ID, domain.ID, domain.Domain, asset.Status, asset.DaysLeft, asset.NotAfter.String())
	return asset, nil
}

func init() {
	packageContext.GET("assets.table", CertAliyunAssetList, CertAliyunAssetListTemplate)
}
