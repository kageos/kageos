...
func joinCertificateNotifyUsers(domain CertAliyunDomain) string {
...
func updateDomainFromCertMeta(db *gorm.DB, domain *CertAliyunDomain, meta certMetadata, checkedAt time.Time) {
	_ = db.Model(&CertAliyunDomain{}).Where("id = ?", domain.ID).Updates(map[string]interface{}{
...
func updateDomainFailure(db *gorm.DB, domainID int, checkedAt time.Time, message string) {
	_ = db.Model(&CertAliyunDomain{}).Where("id = ?", domainID).Updates(map[string]interface{}{
