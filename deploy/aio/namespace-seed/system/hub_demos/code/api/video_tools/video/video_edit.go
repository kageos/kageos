...
func VideoEdit(ctx *app.Context, resp response.Response) error {
	...
	case "添加水印":
		...
		args = []string{
			"-i", file,
			"-vf", fmt.Sprintf("drawtext=text='%s':fontcolor=white:fontsize=24:box=1:boxcolor=black@0.5:boxborderw=5:x=%s:y=%s",
				escapeFFmpegDrawtextText(req.WatermarkText), strings.Split(position, ":")[0], strings.Split(position, ":")[1]),
			"-c:a", "copy",
			"-y", outputPath,
		}
		operationDetails = fmt.Sprintf("添加水印: '%s' (位置: %s)", req.WatermarkText, req.WatermarkPosition)
