...
```bash
cd deploy/customer   # 兼容路径；新部署推荐改用 deploy/prod
cp .env.example .env
# 手写填写 .env（SMTP 相关可留空）
...
| `Dockerfile` / `entrypoint-main.sh` / `nginx/` / `config/prod/` | 兼容镜像与模板路径；官方入口已迁到 `deploy/prod/` |
...
- 用户应用基础镜像的 canonical 构建资源已迁到 **`deploy/base/images/app-base/`**；旧 `customer` 兼容路径构建时也会从这里复制 `Dockerfile` 与 `start.sh`。
- **Podman `runroot must be set`**：镜像内 **`/etc/containers/storage.conf`** 已写 `runroot` / `graphroot`；**`entrypoint-main.sh`** 会创建 **`/run/containers/storage`**。Compose 预检由镜像内空标记文件 **`/etc/ai-agent-os/prod-compose-bundle`** 自动识别；为兼容旧路径也会同时写 **`/etc/ai-agent-os/customer-compose-bundle`**，**线上无需为此设环境变量**。
